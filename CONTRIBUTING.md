...
Thanks for considering a contribution. Framekit is a security-sensitive, headless-friendly CLI for media release preparation; the rules below exist to keep it that way.

## TL;DR

```bash
# 1. Fork + clone
git clone https://github.com/<you>/framekit && cd framekit
...
pytest --cov=src/framekit --cov-report=term-missing
...
Run `fk doctor` to verify your environment.
...
- Tests live in `tests/`. Mirror the source tree: `src/framekit/modules/foo.py` → `tests/test_foo.py`.
...
- Never log raw token, announce URL, password, or other secret material. Use `mask_secret()` from `framekit.modules.metadata.config`.
...
- New external HTTP calls must use the project `httpx` wrapper (`framekit.core.http`) so timeouts, retries, and TLS settings are uniform.
...
- Localized strings go through `framekit.core.i18n.tr()` — never hard-code user-facing English.
- Selector calls go through `framekit.ui.unified_selector` (the consolidation target). Don't add new ad-hoc Click prompts.
