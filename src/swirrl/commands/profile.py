...
from swirrl.core.settings.store import SettingsStore
from swirrl.ui.click_helper import click
from swirrl.ui.console import console, print_error, print_info, print_success
from swirrl.ui.unified_selector import SelectorOption, confirm_choice, text_input
from swirrl.ui.unified_selector import select_many as _select_many
...
def profile_list() -> None:
    ...
    if not profiles:
        print_info("No profiles found. Create one with: swirrl profile create <name>")
        return
...
def profile_use(name: str | None) -> None:
    ...
    profiles = list_profiles()
    if not profiles:
        print_error("No profiles available. Create one first: swirrl profile create <name>")
        return
...
def profile_create(name: str, description: str, from_current: bool, wizard: bool) -> None:
    ...
    Examples:
        swirrl profile create anime -d "Anime workflow with subs"
        swirrl profile create movie-4k --from-current
        swirrl profile create my-preset --wizard
    """
    if from_current:
        store = SettingsStore()
        current = store.load()
