...
from swirrl.modules.encoder import EncoderService, EncoderValidator, PresetLoader
...
def _print_encoder_init_error(exc: Exception) -> None:
    ...
    console.print("     swirrl settings set encoder.ffmpeg_path <path-to-ffmpeg>")
    console.print("  3. Run 'swirrl encode check' to verify installation")
...
def encode_command(
    ...
    Examples:
        swirrl encode run input.mkv --preset films
        swirrl encode run input.mkv --preset-file custom.yaml --output output.mkv
        swirrl encode run folder/ --preset series --recursive --output-dir encoded/
    """
    store = SettingsStore()
    settings = store.load()
    ...
    try:
        ...
        print_info("Use 'swirrl encode list-presets' to see available presets")
        return 1
    except ValueError:
...
def create_preset_command(output: str, preset_type: str) -> int:
    ...
    try:
        ...
        print_info(f"  swirrl encode run input.mkv --preset-file {output_path}")
        return 0
    except Exception as e:
