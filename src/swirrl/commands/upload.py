...
from swirrl.core.cli_helpers import print_fatal
from swirrl.core.paths import get_config_dir
from swirrl.core.settings import SettingsStore, redact_settings
from swirrl.modules.upload import DiscoveryService, TorrentMetadata, UploadService
from swirrl.ui.console import console
from swirrl.ui.timeline import reset_timeline, show_step
...
# ── Timeline helpers (shared from swirrl.ui.timeline) ──────────────────────
...
def assistant_command(
    ...
    env_name = token_env or text_input(
        ...
        default=f"SWIRRL_TRACKER_{_slugify_tracker_name(name).upper().replace('-', '_')}_TOKEN",
    )
...
def _resolve_tracker_selection(
    ...
    if tracker:
        ...
        print_fatal(
            ...
            "Run: swirrl upload list-trackers",
        )
        return None
    if not is_tty:
...
def _infer_release_display_name(torrent_file: Path) -> str:
    ...
    try:
        from swirrl.modules.upload.metadata_extractor import ReleaseParser
...
def _apply_upload_preset(
    ...
    *,
    ...
    if not preset:
        return metadata
    from swirrl.modules.upload.presets import PresetManager
...
def _setup_image_host(steps: list[str]) -> _ImageHostSettings:
    ...
    from swirrl.modules.upload.image_host import ImageHostService
...
def _setup_save_configuration(
    *,
    ...
    from swirrl.modules.upload.models import DiscoveryResult
...
def _print_setup_summary(
    ...
    console.print(
        "\nTest with: [cyan]swirrl upload test --tracker " + credentials.tracker_name + "[/cyan]"
    )
...
def _resolve_run_context(
    ...
    if not trackers:
        print_fatal("No trackers configured", "Run: swirrl upload setup")
        return None
...
def _try_auto_tmdb_id(torrent_file: Path) -> int | None:
    ...
    try:
        ...
        from swirrl.core.settings import SettingsStore
        from swirrl.modules.metadata.factory import build_metadata_provider
        from swirrl.modules.upload.metadata_extractor import ReleaseParser
...
def setup_command():
    ...
    testing the connection, and saving to swirrl.yaml.
    """
    ...
    try:
        ...
        _show_step("Step 5 - Save", "Saving configuration to swirrl.yaml.", "Save", steps)
...
def remove_command():
    ...
    try:
        ...
        if not trackers:
            ...
            console.print("Add one with: [cyan]swirrl upload setup[/cyan]")
            return
...
def list_trackers_command():
    ...
    try:
        ...
        if not trackers:
            console.print("[yellow]No trackers configured[/yellow]")
            console.print("Run: [cyan]swirrl upload setup[/cyan]")
            return
        ...
        print_fatal(str(e), "Check swirrl.yaml for tracker configuration")
...
@click.option("--save/--no-save", default=True, help="Save to swirrl.yaml")
def discover_command(url: str, api_key: str, name: str, tracker_type: str | None, save: bool):
    ...
    \b
    Examples:
      swirrl upload discover --url https://tracker.com --api-key KEY --name mytracker
      swirrl upload discover --url https://tracker.com --api-key KEY --name t --type unit3d
    """
    ...
    try:
        ...
        if save:
            if discovery.save_discovery(result, name, api_key):
                console.print("\n[green]Configuration saved to swirrl.yaml[/green]")
            else:
...
def run_command(
    ...
    \b
    Examples:
      swirrl upload run movie.torrent --tracker bhd --name "Film 2024" \\
          --description "..." --category Films --type 1080p --resolution 1920x1080 --tmdb-id 12345

      swirrl upload run movie.torrent   # fully interactive
      swirrl upload run "E:\\Release\\MyFolder" --tracker my-tracker --dry-run
    """
    try:
...
def stats_command(days: int):
    ...
    from swirrl.modules.upload.stats import UploadStats
...
def preset_add():
    ...
    from swirrl.modules.upload.presets import PresetManager, UploadPreset
...
def queue_list():
    ...
    from swirrl.modules.upload.queue import UploadQueue
