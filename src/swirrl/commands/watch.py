...
from swirrl.core.cli_helpers import print_fatal, print_warning
from swirrl.core.paths import get_config_dir, get_pipeline_presets_dir
from swirrl.core.settings import SettingsStore
from swirrl.modules.watch import WatchConfig, WatcherService
from swirrl.modules.watch.service import (
...
def _validate_enabled_folders_for_start(enabled_folders: list[dict]) -> bool:
    ...
    _show_available_presets()
    console.print(
        "Fix with: [cyan]swirrl watch remove[/cyan] then [cyan]swirrl watch add[/cyan]"
    )
    return False
...
def setup_command():
    ...
    notifications. Saves the result to swirrl.yaml.
    """
    _reset_timeline()
    ...
    try:
        ...
        if folders:
            console.print("Start with: [cyan]swirrl watch start[/cyan]")
        else:
            console.print("Add folders with: [cyan]swirrl watch add[/cyan]")

    except KeyboardInterrupt:
        console.print("\n[yellow]Setup cancelled[/yellow]")
    except Exception as e:
        print_fatal(str(e), "Check that swirrl.yaml is writable")
...
def remove_command():
    ...
    try:
        ...
        if not folders:
            ...
            console.print("Add one with: [cyan]swirrl watch add[/cyan]")
            return
...
def start_command(start_all: bool, no_status_updates: bool, status_interval: int):
    ...
    \b
    Examples:
      swirrl watch start           # choose folders interactively
      swirrl watch start --all     # start all enabled folders
    """
    try:
        ...
        if not enabled_folders:
            ...
            console.print("Add one with: [cyan]swirrl watch add[/cyan]")
            console.print("Or enable folders with: [cyan]swirrl watch list[/cyan]")
            return
        ...
        print_fatal(str(e), "Check your configuration with: swirrl watch test")
...
def status_command(json_output: bool):
    ...
    try:
        ...
        if not folders:
            ...
            console.print("Run: [cyan]swirrl watch add[/cyan]")
            return
        ...
        print_fatal(str(e), "Verify watch configuration in swirrl.yaml")
...
def stop_command(timeout: float) -> None:
    ...
    Looks for ``.swirrl_watch.pid`` next to the active swirrl.yaml and
...
def test_command(folder: Path | None, preset: str | None):
    ...
    Examples:
      swirrl watch test
      swirrl watch test --folder D:\\downloads --preset multi_fr
    """
    try:
        _store, settings, folders = _load_watch_data()
        ...
        if not folders:
            print_fatal(
                "No watch configuration found", "Run 'swirrl watch add' or 'swirrl watch setup'"
            )
            return
        ...
        if all_valid:
            ...
            print_fatal("Configuration has errors - run 'swirrl watch setup' to fix")
