...
from swirrl.core.cache import CacheManager
from swirrl.core.exceptions import SettingsError
from swirrl.core.i18n import tr
from swirrl.core.paths import get_intelligent_cache_dir
...
from swirrl.ui.console import console, print_error, print_success, print_warning
...
def _settings_tables(settings: dict[str, Any]) -> list[Table]:
    ...
    overview = Table(
        title=tr("settings.overview", default="Swirrl overview"),
        box=box.HEAVY,
        expand=True,
    )
...
def run_settings_show(*, json_output: bool = False) -> int:
    ...
    from swirrl.core.settings import redact_settings
...
def run_security_migrate() -> int:
    ...
    try:
        ...
        if not settings_obj.is_security_enabled():
            print_warning("Security is disabled. Enable it first in swirrl.yaml")
            return 1
...
def run_security_reset_vault() -> int:
    ...
    if not settings_obj.is_security_enabled():
        print_warning(
            ...
            "Enable it first with: swirrl settings security toggle --enable"
        )
        return 1
    ...
    console.print("  [cyan]swirrl settings security set-token[/cyan]")
    console.print("  [cyan]swirrl settings security set-announces[/cyan]")
    return 0
...
def _is_vault_mismatch(exc: BaseException) -> bool:
    ...
    from swirrl.core.security.vault import VaultKeyMismatchError
...
def _hint_reset_vault() -> None:
    ...
    console.print(
        ...
        "[cyan]swirrl settings security reset-vault[/cyan]"
    )
    console.print(
        "  • Or disable security entirely: [cyan]swirrl settings security toggle --disable[/cyan]"
    )
...
def run_security_toggle(enable: bool | None) -> int:
    ...
    from swirrl.core.settings import Settings, SettingsStore
    ...
    try:
        ...
        if enable:
            ...
            console.print(
                "[cyan]Tip: Use 'swirrl settings security migrate' to move existing sensitive data to the vault[/cyan]"
            )
        else:
...
@click.group(
    ...
    help=tr(
        "cli.settings.help",
        default=(
            "Inspect Swirrl configuration, cache and security.\n\n"
            "Examples:\n"
            "  swirrl settings                     Show all settings\n"
            "  swirrl settings reset general.locale\n"
            "  swirrl settings cache stats\n"
            "  swirrl settings security status\n\n"
            "Subcommands:\n"
...
def settings_command(ctx: click.Context, json_output: bool) -> int | None:
    """Display or update the active Swirrl settings.
...
def settings_doctor_command(json_output: bool) -> int:
    ...
    from swirrl.commands.doctor import run_doctor_command

    return run_doctor_command(json_output=json_output)


from swirrl.commands.benchmark import benchmark_command  # noqa: E402
