...
from swirrl.modules.renamer.term_selector import (
    TermInventory,
    collect_terms,
    derive_remove_terms,
)
...
def _resolve_multi_language_tags_interactive(
    ...
    *,
    ...
    from swirrl.modules.renamer.rules import replace_language_tag
...
def _run_renamer_json(
    *,
    ...
    """Run the renamer non-interactively and emit a structured JSON result."""

    def _work() -> tuple[int, dict]:
        context = _resolve_renamer_context(path, lang, profile_name)
        if context is None:
            return 1, {"tool": "renamer", "error": "Could not resolve renamer context."}
        folder, service, default_lang, profile, language_profile = context
        _picker_err, effective_remove_terms = _resolve_effective_remove_terms(
            folder=folder,
            remove_terms=remove_terms,
            select_terms=False,
            apply_changes=apply_changes,
        ...
        report = service.run(
            folder,
            default_lang=default_lang,
            apply_changes=apply_changes and not dry_run,
            force_lang=force_lang,
        ...
        files = [
            {
                ...
                "before": detail.before.get("name"),
                "after": detail.after.get("name"),
            }
            for detail in report.details
    ...
    return emit_json_capture(_work)
...
@click.command(
    ...
    help=tr(
        ...
        default=(
            ...
            "Quick examples:\n"
            "  swirrl renamer <folder>                     # Interactive preview and confirm\n"
            "  swirrl ren <folder> --apply                 # Apply without confirmation\n"
            "  swirrl renamer <folder> --lang FRENCH       # Inject language tag\n"
            "  swirrl ren <folder> --remove-term REPACK    # Remove specific terms\n"
            "  swirrl renamer <folder> --select-terms      # Interactive term picker\n\n"
            "Features:\n"
...
@click.option(
    ...
    "auto_mode",
    is_flag=True,
    help="Non-interactive mode — skip term picker and conflict resolution prompts.",
)
@click.option(
    "--json",
    "json_output",
    is_flag=True,
    default=False,
    help="Output a structured JSON rename report to stdout instead of a table.",
)
@click.pass_context
def renamer_command(
    ctx: click.Context,
    ...
    ctx.exit(
        run_renamer_command(
            ...
            remove_terms=remove_terms,
            insert_after_pairs=insert_after_pairs,
            select_terms=False if auto_mode else select_terms,
