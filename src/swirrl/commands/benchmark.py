...
from swirrl.core.performance import get_metrics
...
def _benchmark_import_time(iterations: int) -> dict[str, Any]:
    ...
    for i in range(iterations):
        ...
        modules_to_clear = [m for m in sys.modules if m.startswith("swirrl")]
        for mod in modules_to_clear:
            if mod not in ["swirrl", "swirrl.commands", "swirrl.commands.benchmark"]:
...
@click.command(
    ...
    help=tr(
        "cli.benchmark.help",
        default=(
            "Run performance benchmarks and analyze Swirrl's speed.\n\n"
            ...
            "Quick examples:\n"
            "  swirrl doctor benchmark                     # Run basic benchmarks\n"
            "  swirrl doc benchmark --folder <path>        # Include file scanning tests\n"
            "  swirrl doctor benchmark --iterations 5      # More iterations for accuracy\n"
            "  swirrl doc benchmark --export results.json  # Save results to file\n\n"
            ...
            "Note: This is a subcommand of 'doctor'. Use 'swirrl doctor' for health checks."
        ),
    ),
)
@click.option(
...
def benchmark_command(
    ...
    console.print()
    console.print("[bold cyan]Swirrl Performance Benchmark[/bold cyan]")
    console.print()
