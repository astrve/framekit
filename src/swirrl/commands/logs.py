...
from swirrl.core.diagnostics import default_log_file, get_log_file, redact
    ...
    if path:
        ...
        if not _is_supported_log_path(resolved):
            raise click.ClickException(
                "Unsupported log path. Use swirrl.log, swirrl.log.* or <module>_logs_YYYYMMDD_HHMM.log."
            )
        return resolved
...
def _ensure_primary_log_file(log_file: Path) -> None:
    if log_file.name != "swirrl.log":
        raise click.ClickException("Destructive operations require the primary swirrl.log file.")
...
def _remove_rotated_logs(log_dir: Path, log_file: Path, *, older_than: int) -> int:
    ...
    removed = 0
    for candidate in log_dir.glob("swirrl.log*"):
...
@click.group(
    ...
    help=tr(
        "cli.logs.help",
        default=(
            "Inspect and maintain Swirrl diagnostic logs.\n\n"
            "Subcommands:\n"
...
def logs_clear_command(
    ...
    truncated = False
    if clear_all:
        from swirrl.core.destructive import auto_yes_enabled
...
@logs_command.group(
    "audit",
    help=tr("cli.logs.audit.help", default="Inspect and maintain Swirrl audit log."),
)
...
def logs_audit_show_command(limit: int, as_json: bool) -> None:
    ...
    from swirrl.core.audit_log import read_audit_events
...
def logs_audit_purge_command() -> None:
    ...
    from swirrl.core.audit_log import purge_audit_events
