...
from swirrl.commands.benchmark import benchmark_command
from swirrl.core.diagnostics import diagnostics_summary
...
from swirrl.modules.metadata.config import resolve_metadata_config
...
from swirrl.modules.nfo.templates import list_builtin_templates
...
from swirrl.ui.console import console, print_json
...
def _network_checks(settings: dict) -> list[DoctorCheck]:
    ...
    endpoints = (
        ...
        ("PyPI", "https://pypi.org/pypi/swirrl-auto/json"),
    )
...
def _vault_checks(settings: dict) -> list[DoctorCheck]:
    ...
    for label, path in (
        ...
        if not Path(path).exists():
            ...
            continue
        try:
            from swirrl.core.security.permissions import verify_secure_permissions
        ...
        checks.append(
            DoctorCheck(
                ...
                if secure
                    ...
                    default="loose permissions (run: swirrl settings security repair)",
                ),
            )
        )
...
def _diagnostics_checks() -> list[DoctorCheck]:
    ...
    return [
        DoctorCheck(
            ...
            status="warn" if summary.get("debug") else "ok",
            detail=tr("common.enabled", default="enabled")
...
@click.group(
    ...
    help=tr(
        ...
        default=(
            ...
            "The doctor command performs health checks on your Swirrl installation, verifying "
            "external tools, settings, metadata configuration, templates, and more.\n\n"
            "Quick examples:\n"
            "  swirrl doctor                               # Run all checks\n"
            "  swirrl diag -j                              # Output as JSON\n"
            "  swirrl doc                                  # Quick health check\n"
            "  swirrl doctor benchmark                     # Run performance benchmarks\n\n"
            "Checks performed:\n"
