...
from swirrl.core.i18n import set_locale, tr
...
from swirrl.core.settings import DEFAULT_SETTINGS, SettingsStore, normalize_ui_locale
...
from swirrl.modules.nfo.template_selector import build_template_options, choose_template
...
from swirrl.modules.setup.selector import ChoiceOption, choose_option
...
def _choose_workspace_path(
    ...
    _show_step(
        ...
        tr(
            ...
            default=(
                "Choose where Swirrl should start when no path is passed to the command.\n\n"
                "AppData Workspace: managed by Swirrl in your user profile.\n"
                "Swirrl Project Workspace: inside your current Swirrl project.\n"
    ...
    while True:
        ...
        options = [
            ...
            ChoiceOption(
                "project",
                tr("setup.workspace.project", default="Swirrl Project Workspace"),
                str(project_path),
            ),
            ChoiceOption(
                "custom",
...
def _choose_interface_language(current: str) -> str:
    _show_step(
        ...
        tr(
            ...
            default=(
                "This controls the language used by Swirrl CLI messages.\n\n"
...
def _choose_metadata_language(current: str) -> str:
    _show_step(
        ...
        tr(
            ...
            default=(
                "This controls the language used when Swirrl requests metadata online.\n\n"
...
def _prompt_tmdb_token(current_token: str) -> str | None:
    _show_step(
        ...
        tr(
            ...
            default=(
                "Swirrl uses this token to search movie and episode metadata.\n\n"
...
def _choose_builtin_template(current_template: str) -> str:
    ...
    from swirrl.modules.nfo.template_registry import NfoTemplateRegistry
...
def _print_setup_summary(settings: dict) -> None:
    table = Table(
        title=tr("setup.summary_title", default="Swirrl Setup Summary"),
        expand=True,
        box=box.HEAVY,
        border_style="white",
    )
    ...
    console.print(
        Panel(
            tr(
                "setup.welcome_body",
                default=(
                    "Welcome to Swirrl guided setup.\n\n"
            ...
            title=tr("setup.welcome_title", default="Swirrl Setup"),
            border_style="white",
            box=box.HEAVY,
            expand=True,
        )
...
def _choose_settings_storage_path(current_path: Path) -> Path:
    _show_step(
        tr("setup.storage_title", default="Swirrl settings storage"),
        tr(
            "setup.storage_body",
            default=(
                "Choose where Swirrl should save its global configuration.\n\n"
                "Recommended: the OS user config directory.\n"
                "Custom folder: useful for a dedicated disk or portable setup.\n\n"
                "Project configs still stay opt-in with 'swirrl init'."
            ),
        ),
    ...
    while True:
        ...
        custom = _prompt_custom_path(
            tr("setup.storage.custom_label", default="Swirrl config folder"),
...
def _run_storage_step() -> SettingsStore:
    ...
    if os.environ.get(ENV_CONFIG_PATH, "").strip():
        print_warning(
            tr(
                ...
                default=(
                    "SWIRRL_CONFIG is set. It overrides setup storage choices. "
    ...
    print_success(
        tr(
            "setup.storage.saved",
            default="Swirrl settings path configured: {path}",
...
def _run_security_step(settings: dict) -> None:
    ...
    _show_step(
        ...
        tr(
            "setup.security_body",
            default=(
                "Swirrl can encrypt sensitive data (TMDb tokens, torrent announce URLs)\n"
                "using a secure vault stored in your system keyring.\n\n"
                "Recommended: enabled for better security.\n"
                "Note: You can change this later with 'swirrl settings security toggle'."
            ),
        ),
...
def _run_metadata_step(settings: dict, store: SettingsStore) -> None:
    ...
    _show_step(
        ...
        tr(
            ...
            default=(
                "When enabled, Swirrl lets you review and confirm the metadata match\n"
...
def _save_setup(settings: dict, store: SettingsStore, mark_completed: bool) -> int:
    ...
    store.save(settings)
    print_success(tr("setup.success.saved", default="Swirrl setup saved."))
    return 0
...
@click.command(
    ...
    help=tr(
        "cli.setup.help",
        default=(
            "Run the Swirrl setup wizard.\n\n"
            "The setup command now uses the wizard flow only, with Normal and Advanced modes.\n\n"
            "Quick examples:\n"
            "  swirrl setup                                # Wizard in Normal mode\n"
            "  swirrl setup --mode advanced                # Wizard in Advanced mode\n"
            "  swirrl init                                 # Same as setup\n\n"
...
def setup_command(mode: str, wizard: bool, profile: str | None) -> int:
    ...
    from swirrl.modules.setup import SetupWizard, WizardCancelled
