...
from swirrl.core.http import redact_url
from swirrl.core.i18n import tr
from swirrl.core.paths import PathResolver
from swirrl.core.settings import SettingsStore
from swirrl.modules.nfo.formatting import format_bytes_human
from swirrl.modules.torrent.payload import (
    TorrentPayload,
    discover_torrent_payload_candidates,
    resolve_torrent_payload,
)
from swirrl.modules.torrent.service import (
    TorrentBuildOptions,
    TorrentService,
...
from swirrl.ui.progress import swirrl_progress
from swirrl.ui.unified_selector import SelectorOption, confirm_choice
...
def _print_vault_fix_hint() -> None:
    print_info(
        tr(
            ...
            default="Solutions:\n"
            "  1. Run 'swirrl settings security status' to check vault status\n"
            "  2. Run 'swirrl settings security set-announces <url>' to reconfigure\n"
            "  3. Run 'swirrl setup' to restore vault access",
        )
    )
            ...
            entries=[
                ...
                SelectorOption(
                    ...
                    hint=tr(
                        ...
                        default="Désactiver cette question dans swirrl.yaml.",
                    ),
                ),
            ],
            page_size=6,
...
def _prompt_for_announce_and_save(settings: dict) -> str:
    ...
    Triggered when the user runs ``swirrl torrent`` without any announce
        ...
        print_error(
            tr(
                ...
                default=(
                    ...
                    "'swirrl settings security set-announces' (with security "
        ...
        if is_encrypted:
            print_error(
                tr(
                    ...
                    default="Announce URLs are encrypted but vault is unavailable. Run `swirrl setup` to restore access.",
                )
            )
        else:
...
@click.command(
    ...
    help=tr(
        ...
        default=(
            ...
            "Quick examples:\n"
            "  swirrl torrent <folder>                     # Auto-detect media payload\n"
            "  swirrl tor <folder> --announce <url>        # Specify tracker URL\n"
            "  swirrl torrent <folder> --select-content    # Interactive payload selection\n"
            "  swirrl tor <folder> --content media         # Force media-only mode\n"
            "  swirrl torrent --set-announce <url>         # Save default announce URL\n\n"
            "Setup:\n"
            "  1. Add tracker announce URL: swirrl tor --add-announce <url>\n"
            "  2. Select default: swirrl tor --select-announce\n"
            "  3. Create torrent: swirrl tor <folder>\n\n"
            "Features:\n"
