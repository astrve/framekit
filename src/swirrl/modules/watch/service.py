...
# Project-local PID file. Lives next to the active swirrl.yaml so a stop
# command issued from the same working directory always targets the right
# instance.
WATCH_PID_FILENAME = ".swirrl_watch.pid"
    ...
    def __init__(
        ...
        Args:
            ...
            embedded: When *True* the service runs inside a larger host process
                (e.g. ``swirrl serve``).  Three behaviours change:

                * Signal handlers are **not** installed — the host process owns
                  its own signal lifecycle.
                * The ``.swirrl_watch.pid`` file is **not** written or removed
                  so that ``swirrl watch stop`` cannot accidentally terminate
                  the host service process.
                * ``start()`` returns immediately after launching background
                  threads instead of blocking in a ``while`` loop.
            job_enqueuer: Optional callable ``(path, preset, rule_id) -> job_id``.
                When provided (service mode), detected files are enqueued as
                real jobs instead of being processed in-process.
            event_emitter: Optional callable ``(path, preset, rule_id) -> None``.
                Called on each validated detection to emit service events and
                webhooks.
        """
        self.config = config
    ...
    def _process_queue(self) -> None:
        ...
        while not self.stop_event.is_set():
            try:
                ...
                # Enforce watch rule filename pattern when set.
                if folder_config.pattern:
                    try:
                        if not _re.search(folder_config.pattern, file_path.name):
                            logger.debug(
                                f"Pattern '{folder_config.pattern}' no match, skipping: {file_path.name}"
                            )
                            continue
                    except _re.error as exc:
                        logger.warning(f"Invalid watch rule pattern '{folder_config.pattern}': {exc}")
                ...
                # Emit detection event before processing.
                if self._event_emitter is not None:
                    try:
                        self._event_emitter(str(file_path), folder_config.preset, folder_config.id)
                    except Exception as exc:
                        logger.warning(f"Watch event emitter error: {exc}")

                if self._job_enqueuer is not None:
                    # Embedded service mode: enqueue as a real observable job.
                    release_folder = str(file_path.parent)
                    # Kind routing: resolve effective preset before enqueueing.
                    effective_preset = folder_config.preset
                    if folder_config.kind_routing == "auto":
                        try:
                            import re as _re2
                            mkv_files = list(file_path.parent.glob("*.mkv"))
                            ep_pat = _re2.compile(r'[Ss]\d{1,2}[Ee]\d{1,2}')
                            name = file_path.parent.name
                            if ep_pat.search(name) or any(ep_pat.search(f.name) for f in mkv_files):
                                kind = "single_episode"
                            elif len(mkv_files) > 1:
                                kind = "series"
                            else:
                                kind = "movie"
                            pbk = folder_config.presets_by_kind or {}
                            kind_preset = pbk.get(kind) or pbk.get("movie") or effective_preset
                            if kind_preset:
                                effective_preset = kind_preset
                        except Exception as _kexc:
                            logger.debug(f"Kind detection failed for {file_path}: {_kexc}")
                    try:
                        job_id = self._job_enqueuer(
                            release_folder, effective_preset, folder_config.id
                        )
                        logger.info(f"Watch: enqueued job {job_id} for {file_path.name}")
                        self.status.files_processing -= 1
                        self.status.files_processed += 1
                    except Exception as exc:
                        logger.error(f"Watch: failed to enqueue job for {file_path.name}: {exc}")
                        self.status.files_processing -= 1
                        self.status.files_failed += 1
                else:
                    # Standalone CLI: in-process execution (unchanged behaviour).
