...
def _env_metadata_values(env: Mapping[str, str]) -> dict[str, str]:
    return {
        "tmdb_read_access_token": env.get("SWIRRL_TMDB_READ_ACCESS_TOKEN", "").strip(),
        "tvdb_api_key": env.get("SWIRRL_TVDB_API_KEY", "").strip(),
        "trakt_client_id": env.get("SWIRRL_TRAKT_CLIENT_ID", "").strip(),
        "trakt_client_secret": env.get("SWIRRL_TRAKT_CLIENT_SECRET", "").strip(),
        "trakt_access_token": env.get("SWIRRL_TRAKT_ACCESS_TOKEN", "").strip(),
        "language": env.get("SWIRRL_METADATA_LANGUAGE", "").strip(),
    }
...
def _resolve_tmdb_secret(value: str) -> str:
    ...
    try:
        ...
        logger.warning(
            ...
            "Run 'swirrl settings security set-token' to configure the token."
        )
        ...
        logger.warning(
            "TMDB token decryption failed. Run 'swirrl settings security set-token' to "
            "reconfigure, or 'swirrl settings security status' to check vault status."
        )
    return ""  # nosec B105
...
def _resolve_tvdb_secret(value: str) -> str:
    ...
    logger.warning(
        ...
        "Please provide the key directly in swirrl.yaml or via SWIRRL_TVDB_API_KEY environment variable."
    )
    return ""
