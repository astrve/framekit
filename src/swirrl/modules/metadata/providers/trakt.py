...
from swirrl.modules.metadata.providers.trakt_models import TraktIds, TraktMovie, TraktShow
...
class TraktProvider(MetadataProvider):
    ...
    def _make_request(
        ...
        try:
            ...
            raise SwirrlMetadataError(f"Trakt authentication failed: {e}") from e
        except HttpRateLimitError:
            raise
        except HttpStatusError as e:
            if e.status_code == 404:
                return None
            raise SwirrlMetadataError(f"Trakt API error: {e}") from e
        except HttpError as e:
            raise SwirrlMetadataError(f"Trakt request error: {e}") from e
    ...
    def fetch_movie(self, candidate: MetadataCandidate) -> MovieMetadata:
        ...
        Raises:
            SwirrlMetadataError: If fetch fails
        """
        ...
        try:
            ...
            if not data:
                raise SwirrlMetadataError(f"Trakt movie not found: {candidate.provider_id}")
            ...
            raise SwirrlMetadataError(f"Failed to fetch Trakt movie: {e}") from e
    ...
    def fetch_episode(self, candidate: MetadataCandidate) -> EpisodeMetadata:
        ...
        try:
            ...
            if not show_data:
                raise SwirrlMetadataError(f"Trakt show not found: {candidate.provider_id}")
            ...
            raise SwirrlMetadataError(f"Failed to fetch Trakt episode: {e}") from e
    ...
    def fetch_season(self, candidate: MetadataCandidate) -> SeasonMetadata:
        ...
        try:
            ...
            raise SwirrlMetadataError(f"Failed to fetch Trakt season: {e}") from e
