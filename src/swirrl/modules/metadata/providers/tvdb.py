...
It handles authentication, rate limiting, and data transformation to Swirrl's
...
from swirrl.core.http import HttpAuthError, HttpClient, HttpError, HttpRateLimitError
...
class TVDBProvider(MetadataProvider):
    ...
    def _authenticate(self) -> None:
        ...
        Raises:
            SwirrlMetadataError: If authentication fails
        """
        ...
        try:
            ...
            if not self.token:
                raise SwirrlMetadataError("TVDB authentication failed: no token in response")

            logger.debug("TVDB authentication successful")

        except HttpAuthError as e:
            raise SwirrlMetadataError(f"TVDB authentication failed: {e}") from e
        except HttpError as e:
            raise SwirrlMetadataError(f"TVDB authentication error: {e}") from e
    ...
    def _make_request(
        ...
        Raises:
            SwirrlMetadataError: On request failure
        ...
        try:
            ...
            raise SwirrlMetadataError(f"TVDB API request failed: {e}") from e
    ...
    def fetch_movie(self, candidate: MetadataCandidate) -> MovieMetadata:
        ...
        Raises:
            SwirrlMetadataError: TVDB does not support movies
        """
        raise SwirrlMetadataError("TVDB does not support movie metadata")
    ...
    def fetch_episode(self, candidate: MetadataCandidate) -> EpisodeMetadata:
        ...
        Raises:
            SwirrlMetadataError: On fetch failure
        """
        series_id, season_num, episode_num = self._resolve_episode_lookup(candidate)
        if season_num is None or episode_num is None:
            raise SwirrlMetadataError("Season and episode numbers required for episode fetch")
        ...
        try:
            ...
            raise
        except SwirrlMetadataError:
            raise
        except Exception as e:
            logger.error(f"Failed to fetch TVDB episode: {e}")
            raise SwirrlMetadataError(f"Failed to fetch TVDB episode: {e}") from e
    ...
    def fetch_season(self, candidate: MetadataCandidate) -> SeasonMetadata:
        ...
        if not season_num:
            raise SwirrlMetadataError("Season number required for season fetch")
        ...
        try:
            ...
            raise SwirrlMetadataError(f"Failed to fetch TVDB season: {e}") from e
