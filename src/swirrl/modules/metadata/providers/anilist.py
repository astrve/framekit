...
It handles GraphQL queries, rate limiting, and data transformation to Swirrl's
...
from swirrl.core.http import HttpClient, HttpError, HttpRateLimitError, HttpStatusError
...
from swirrl.modules.metadata.providers.anilist_models import AniListMedia
...
class AniListProvider(MetadataProvider):
    ...
    def _make_graphql_request(self, query: str, variables: dict[str, Any]) -> dict[str, Any]:
        ...
        Raises:
            SwirrlMetadataError: If request fails
        """
        # Apply rate limiting
        if not self.rate_limiter.acquire(timeout=30.0):
            raise SwirrlMetadataError("AniList rate limit exceeded")
        ...
        try:
            ...
            if "errors" in data:
                ...
                raise SwirrlMetadataError(f"AniList GraphQL errors: {', '.join(error_messages)}")

            return data

        except HttpRateLimitError as e:
            raise SwirrlMetadataError(f"AniList rate limit error: {e}") from e
        except HttpStatusError as e:
            raise SwirrlMetadataError(f"AniList API error: {e}") from e
        except HttpError as e:
            raise SwirrlMetadataError(f"AniList request error: {e}") from e
    ...
    def search(self, request: MetadataLookupRequest) -> list[MetadataCandidate]:
        ...
        try:
            ...
            raise SwirrlMetadataError(f"AniList search failed: {e}") from e
    ...
    def _fetch_media_details(self, media_id: str) -> AniListMedia:
        ...
        try:
            ...
            raise SwirrlMetadataError(f"Invalid AniList media ID: {media_id}") from e
        ...
        try:
            ...
            if not media_data:
                raise SwirrlMetadataError(f"AniList media not found: {media_id}")
            ...
            raise SwirrlMetadataError(f"Failed to fetch AniList media {media_id}: {e}") from e
