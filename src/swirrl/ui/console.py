...
from swirrl.ui.branding import swirrl_theme
...
console = Console(theme=swirrl_theme, legacy_windows=False)

# Export accent color for selector
from swirrl.ui.branding import Colors  # noqa: E402

ACCENT = Colors.SWIRRL_ACCENT_PRIMARY
...
def print_success(message: str) -> None:
    ...
    if not _is_quiet_mode():
        from swirrl.ui.branding import StatusIndicators
...
def print_exception_error(exc: BaseException, *, message: str | None = None) -> None:
    ...
    from swirrl.ui.branding import Colors
...
def emit_json_capture(work: Callable[[], tuple[int, dict[str, Any]]]) -> int:
    """Run ``work`` with stdout muted, then print its JSON payload to real stdout.

    ``work`` returns ``(exit_code, payload)``. Used by ``--json`` command modes so
    the subprocess stdout is a single JSON object (the web job runner detects a
    pure-JSON stdout and exposes it as the job's structured ``parsed_payload``).
    Any console output produced during ``work`` is redirected to a buffer and
    dropped, guaranteeing stdout stays valid JSON.
    """
    ...
    try:
        ...
        code = 1
    body: dict[str, Any] = {"ok": code == 0, "returncode": code}
    body.update(payload)
    print(json.dumps(body, ensure_ascii=False), file=real_stdout)
    return code
...
def format_module_header(module_name: str) -> str:
    ...
    from swirrl.ui.branding import get_module_color
...
def format_status_message(
    ...
    from swirrl.ui.branding import format_status
...
def print_module_status(
    ...
    from swirrl.ui.branding import format_status, get_module_color
