...
Provides a standard way to present choices to users across all Swirrl
...
from swirrl.ui.console import ACCENT, console
from swirrl.ui.selector import (
    SelectorAction,
    SelectorDivider,
    SelectorEntry,
...
def _get_checkpoint_dir() -> "Path | None":
    """Return checkpoint dir when this process is a checkpointable web job."""
    import os
    from pathlib import Path

    d = os.environ.get("SWIRRL_JOB_CHECKPOINT_DIR", "").strip()
    if not d:
        return None
    p = Path(d)
...
def _file_checkpoint_select(
    title: str,
    entries: "Sequence[SelectorEntry]",
    checkpoint_dir: "Path",
) -> object | None:
    """Write a checkpoint request file and poll for a web UI response.

    Returns the selected value, or None on timeout — caller falls through to headless auto-pick.

    Checkpoint protocol (files in checkpoint_dir):
      pending.json  — written by this function:
        { type: "select_one", title: str, options: [{index, label, hint}], default_index: int }
      response.json — written by the web backend on user selection:
        { selection_index: int }
    """
    import json
    import time
    ...
    options = [
        {"index": i, "label": e.label, "hint": getattr(e, "hint", "") or ""}
        for i, e in enumerate(entries)
        if isinstance(e, SelectorOption)
    ]
    if not options:
        return None

    preselected = next(
        (i for i, e in enumerate(entries) if isinstance(e, SelectorOption) and getattr(e, "selected", False)),
        0,
    )

    pending_file = checkpoint_dir / "pending.json"
    response_file = checkpoint_dir / "response.json"
    ...
    while time.monotonic() < deadline:
        if response_file.exists():
            try:
                ...
                idx = int(data.get("selection_index", preselected))
                valid = [e for e in entries if isinstance(e, SelectorOption)]
                if 0 <= idx < len(valid):
...
def select_one(*args, **kwargs):  # type: ignore[no-untyped-def]
    ...
    if not sys.stdin.isatty() or auto_mode:
        ...
        if chk_dir is not None:
            result = _file_checkpoint_select(title, entries, chk_dir)
