...
PROJECT_REPO_LABEL = "github.com/astrve/swirrl"
COPYRIGHT_YEARS = "2026"


# Swirrl Visual Identity - Color Palette (Based on SWIRRL_VISUAL_IDENTITY_SPEC.md)
class Colors:
    """Swirrl color palette - Modern Minimalist Premium."""

    # Primary Brand Colors
    SWIRRL_SLATE_DARK = "#2C3E50"  # Main brand color, headers
    SWIRRL_SLATE_MEDIUM = "#34495E"  # Secondary elements
    SWIRRL_SLATE_LIGHT = "#7F8C8D"  # Muted text, borders
    SWIRRL_ACCENT_PRIMARY = "#5DADE2"  # Links, interactive elements
    SWIRRL_ACCENT_SECONDARY = "#85C1E9"  # Hover states, subtle highlights
    ...
    PRIMARY = SWIRRL_ACCENT_PRIMARY
    SUCCESS = SUCCESS_GREEN
    WARNING = WARNING_AMBER
    ERROR = ERROR_RED
    INFO = INFO_BLUE
    ACCENT = SWIRRL_ACCENT_PRIMARY
    MUTED = MEDIUM_GRAY
    HIGHLIGHT = SWIRRL_ACCENT_SECONDARY

    # Status colors
    PENDING = MEDIUM_GRAY
    IN_PROGRESS = SWIRRL_ACCENT_PRIMARY
    COMPLETE = SUCCESS_GREEN
    FAILED = ERROR_RED
    SKIPPED = MEDIUM_GRAY

    # Module-specific colors (using new palette)
    RENAMER = SWIRRL_ACCENT_PRIMARY
    CLEANMKV = INFO_BLUE
    NFO = SWIRRL_SLATE_MEDIUM
    PREZ = WARNING_AMBER
    TORRENT = SUCCESS_GREEN
    CHECKSUM = INFO_BLUE_LIGHT
    METADATA = SWIRRL_ACCENT_SECONDARY
    PIPELINE = SWIRRL_ACCENT_PRIMARY


# Swirrl Theme for Rich Console
swirrl_theme = Theme(
    {
        # Brand Colors
        "swirrl.primary": Colors.SWIRRL_SLATE_DARK,
        "swirrl.accent": Colors.SWIRRL_ACCENT_PRIMARY,
        "swirrl.muted": Colors.SWIRRL_SLATE_LIGHT,
        # Status Colors
        ...
        "status.processing": Colors.SWIRRL_ACCENT_PRIMARY,
        "status.done": f"{Colors.SUCCESS_GREEN} bold",
        # Text Styles
        "header.main": f"{Colors.SWIRRL_SLATE_DARK} bold",
        "header.section": f"{Colors.SWIRRL_SLATE_MEDIUM} bold",
        "header.subsection": f"{Colors.SWIRRL_ACCENT_PRIMARY} bold",
        "text.body": "white",
        "text.secondary": Colors.LIGHT_GRAY,
        "text.code": Colors.SWIRRL_ACCENT_SECONDARY,
        # UI Elements
        "separator": Colors.SWIRRL_SLATE_LIGHT,
        "progress.bar": Colors.SWIRRL_ACCENT_PRIMARY,
...
def build_logo_text() -> Text:
    ...
    for index, line in enumerate(SWIRRL_LOGO_LINES):
        style = f"bold {LOGO_ACCENT}" if index == 0 else f"bold {LOGO_PRIMARY}"
        text.append(line, style=style)

        if index < len(SWIRRL_LOGO_LINES) - 1:
            text.append("\n")
...
def build_meta_text() -> Text:
    """Build meta text."""
    total_width = _art_width(SWIRRL_LOGO_LINES)
...
def print_module_banner(module_name: str | None = None) -> None:
    """Print the Swirrl banner with optional module name.

    Suppressed when ``SWIRRL_WEB_JOB=1`` is set in the environment so that
    web-runner subprocesses produce clean output without the ASCII logo header.
    """
    import os

    if os.environ.get("SWIRRL_WEB_JOB"):
        return
