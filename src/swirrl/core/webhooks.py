"""Webhook dispatch for Swirrl events.
...
def _build_discord_payload(
    ...
    *,
    ...
    if title_template:
        ...
        embed_title = f"Swirrl — {event.replace('.', ' ').title()}"
    ...
    if body_template:
        ...
        if event.startswith("job."):
            ...
            args = context["args"]
            cmd = f"`swirrl {module}{' ' + args if args else ''}`"
    ...
    try:
        ...
        with httpx.Client(timeout=10.0) as client:
            response = client.post(
                webhook.url,
                json=payload,
                headers={"Content-Type": "application/json", "User-Agent": "Swirrl/2.0"},
            )
