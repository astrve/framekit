...
class SwirrlError(Exception):
    """Base exception for user-facing Swirrl failures.
    ...
    def with_suggestions(self, *hints: str) -> SwirrlError:
        """Return ``self`` after appending recovery hints.

        Helper for raising sites that want a fluent style::

            raise SwirrlConfigError("vault not initialised").with_suggestions(
                "Run: swirrl settings security enable",
                "Then: swirrl settings security set-token",
            )
        """
...
class SettingsError(SwirrlConfigError):
    """Raised when settings cannot be read, written, or validated."""


class ToolError(SwirrlExternalToolError):
    """Raised when an external tool cannot be resolved or executed."""


class ValidationError(SwirrlUserInputError):
