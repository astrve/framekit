...
from swirrl.core.exceptions import SwirrlHttpError
...
def _swirrl_user_agent() -> str:
    """Return ``Swirrl/<version>`` using the installed package metadata."""
    try:
        from importlib.metadata import PackageNotFoundError
        from importlib.metadata import version as pkg_version
    except ImportError:  # pragma: no cover - importlib.metadata is stdlib >= 3.8
        return "Swirrl/unknown"
    try:
        return f"Swirrl/{pkg_version('swirrl-auto')}"
    except PackageNotFoundError:
        return "Swirrl/unknown"


DEFAULT_USER_AGENT = _swirrl_user_agent()
...
class HttpClientConfig:
    ...
    user_agent: str = field(default_factory=_swirrl_user_agent)
...
class HttpError(SwirrlHttpError):
    """HttpError raised on swirrl failures."""
...
def _classify_connect_error(exc: httpx.ConnectError, url: str) -> HttpNetworkError:
    """Translate ``httpx.ConnectError`` into the right Swirrl exception."""
