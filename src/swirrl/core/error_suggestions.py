...
in :mod:`swirrl.__main__` consults this when an exception lacks its own
...
from swirrl.core.exceptions import (
    SwirrlConfigError,
    SwirrlError,
    SwirrlExternalToolError,
    SwirrlHttpError,
    SwirrlMetadataError,
    SwirrlUserInputError,
)
...
_RULES: tuple[_Rule, ...] = (
    _Rule(
        ...
        suggestions=(
            ...
            "Run: swirrl metadata --token (paste the JWT-shaped value)",
            "Or export it: SWIRRL_TMDB_READ_ACCESS_TOKEN=eyJ...",
        ),
        type_filter=(SwirrlMetadataError, SwirrlConfigError),
    ),
    _Rule(
    ...
    _Rule(
        ...
        suggestions=(
            ...
            "Run: swirrl doctor to verify network connectivity",
        ),
        type_filter=(SwirrlHttpError,),
    ),
    _Rule(
    ...
    _Rule(
        ...
        suggestions=(
            ...
            "Run: swirrl doctor to confirm the tools are visible on PATH",
        ),
        type_filter=(SwirrlExternalToolError,),
    ),
    _Rule(
        keywords=("schema_version", "unsupported ui locale", "unsupported metadata provider"),
        suggestions=(
            "Open swirrl.yaml and confirm schema_version matches the installed version",
            "Reset the file with: swirrl init --force (backs up nothing — copy yours first)",
            "Run: swirrl doctor for a guided diagnosis",
        ),
        type_filter=(SwirrlConfigError,),
    ),
    _Rule(
        keywords=("vault", "encryption manager", "key file"),
        suggestions=(
            "Enable security: swirrl settings set security.enabled true",
            "Re-create the vault: swirrl settings security init",
            "Move the token back into the vault: swirrl metadata --token",
        ),
        type_filter=(SwirrlConfigError,),
    ),
    _Rule(
        ...
        suggestions=(
            "List valid keys with: swirrl settings show",
            "Check the schema in swirrl.example.yaml (shipped with the release)",
        ),
        type_filter=(SwirrlUserInputError, SwirrlConfigError),
    ),
    _Rule(
    ...
    _Rule(
        ...
        suggestions=(
            ...
            "See: swirrl doctor → Settings → path_validation.",
        ),
    ),
)
