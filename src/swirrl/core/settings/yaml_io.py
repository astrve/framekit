...
def _generate_yaml_with_comments(data: dict[str, Any]) -> str:  # pyright: ignore[reportUnusedFunction]  # Re-exported through package __init__
    ...
    # Header comments
    lines.append("# Swirrl Configuration")
    lines.append("# https://github.com/astrve/swirrl")
    lines.append("")
    ...
    lines.append(
        f"  torrent_client_category: {_yaml_quote(upload_data.get('torrent_client_category', 'swirrl'))}"
    )
