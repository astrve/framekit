...
class SettingsStore:
    """Low-level persistence for ``swirrl.yaml``.

    Reads/writes the active project YAML, applies schema migrations on the
    fly, and locks the file with ``filelock`` so concurrent ``swirrl``
    ...
    * **First write** (file does not exist): uses the hand-rolled emitter in
      :mod:`swirrl.core.settings.yaml_io` which adds section comments
      throughout the file. The output matches the layout
      shipped in ``swirrl.example.yaml`` so a fresh install reads exactly
      like the docs.
