"""Windows service / scheduled-task management for Swirrl.

Provides helpers for installing, uninstalling, starting, stopping, and
querying the Swirrl service on Windows.
...
_SERVICE_NAME = "Swirrl"
_DISPLAY_NAME = "Swirrl Web Service"
_DESCRIPTION = "Swirrl release-workflow web service and file watcher."
_TASK_NAME = "Swirrl"
...
def install_nssm(
    ...
    """Install the Swirrl Windows service via NSSM.
    ...
    Args:
        ...
        host:       Bind host for `swirrl serve`.
        port:       Bind port for `swirrl serve`.
    ...
    steps: list[tuple[str, list[str]]] = [
        (
            ...
            [nssm, "install", _SERVICE_NAME, python_exe,
             "-m", "swirrl", "serve", "--host", host, "--port", str(port)],
        ),
        (
            "set display name",
...
def install_sc(python_exe: str, host: str, port: int) -> tuple[bool, str]:
    """Install the Swirrl Windows service via sc.exe (no log redirection).
    ...
    binpath = f'"{python_exe}" -m swirrl serve --host {host} --port {port}'
    ok, out = _run([
...
def _write_task_bat(
    ...
    ``swirrl service logs`` works in task mode.
    """
    line = (
        f'"{python_exe}" -m swirrl serve --host {host} --port {port}'
        f' >> "{log_out}" 2>> "{log_err}"'
    )
    content = "@echo off\r\n" + line + "\r\n"
...
def install_task(
    ...
    """Install a scheduled task that starts Swirrl at user log-on.
    ...
    return True, (
        ...
        "Swirrl will start automatically at next log-on. "
...
def uninstall_nssm(nssm: str) -> tuple[bool, str]:
    """Remove the NSSM-managed Swirrl service."""
...
def uninstall_sc() -> tuple[bool, str]:
    """Remove the sc.exe-managed Swirrl service."""
    guard = _windows_only()
    if guard is not None:
        return guard
...
def uninstall_task() -> tuple[bool, str]:
    """Remove the scheduled task for Swirrl."""
    guard = _windows_only()
    if guard is not None:
        return guard
...
def start_service_sc() -> tuple[bool, str]:
    """Start the Swirrl Windows service via sc.exe."""
    guard = _windows_only()
    if guard is not None:
        return guard
...
def stop_service_sc() -> tuple[bool, str]:
    """Stop the Swirrl Windows service via sc.exe."""
    guard = _windows_only()
    if guard is not None:
        return guard
...
def start_task(pid_path: Path | None = None) -> tuple[bool, str]:
    """Start the Swirrl scheduled task via schtasks."""
    guard = _windows_only()
    if guard is not None:
        return guard
...
def stop_task(pid_path: Path | None = None) -> tuple[bool, str]:
    """Stop the Swirrl scheduled task via schtasks.
...
def query_sc_status() -> dict[str, str]:
    """Return a status dict from `sc query Swirrl`.
...
def query_task_status() -> dict[str, str]:
    """Return a status dict from `schtasks /Query` for the Swirrl task.
