"""Linux user-service management for Swirrl via systemd --user.

Provides helpers for installing, uninstalling, starting, stopping, and
querying a Swirrl service unit in ``~/.config/systemd/user``.
...
_UNIT_NAME = "swirrl.service"
_DESCRIPTION = "Swirrl Web Service"
...
def _build_exec_start(host: str, port: int) -> str:
    """Return ExecStart command using ``swirrl serve``."""
    swirrl_exe = shutil.which("swirrl")
    host_arg = shlex.quote(host)
    if swirrl_exe:
        return (
            f"{shlex.quote(swirrl_exe)} serve "
            f"--host {host_arg} --port {int(port)}"
        )
    return (
        f"{shlex.quote(sys.executable)} -m swirrl serve "
        f"--host {host_arg} --port {int(port)}"
    )
...
def _is_missing_unit_message(message: str) -> bool:
    ...
    patterns = (
        ...
        "not-found",
        "no such file",
        "unit swirrl.service could not be found",
        "unit swirrl.service not found",
    )
...
def install_systemd_user(
    ...
    """Install swirrl user-service unit and enable at login."""
    guard = _linux_only()
...
def uninstall_systemd_user(unit_dir: Path | None = None) -> tuple[bool, str]:
    """Disable and remove swirrl user-service unit."""
    guard = _linux_only()
    if guard is not None:
        return guard
...
def query_systemd_status() -> dict[str, str]:
    """Return status from ``systemctl --user show swirrl.service``."""
    guard = _linux_only()
    if guard is not None:
...
def journal_logs(lines: int = 50, follow: bool = False) -> tuple[bool, str]:
    """Return/tail journal logs for swirrl user unit."""
    guard = _linux_only()
    if guard is not None:
        return guard
