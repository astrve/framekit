...
SETTINGS_FILENAME = "swirrl.yaml"
LEGACY_SETTINGS_FILENAME = "settings.json"
SETTINGS_POINTER_FILENAME = "settings-path.txt"
ENV_CONFIG_PATH = "SWIRRL_CONFIG"
ENV_CONFIG_DIR = "SWIRRL_CONFIG_DIR"
ENV_CACHE_DIR = "SWIRRL_CACHE_DIR"
...
def list_legacy_config_dirs() -> list[Path]:
    """Return sibling config directories that look like prior app state.

    Detection is intentionally conservative: only directories with multiple
    Swirrl-like config markers are flagged, and no automatic migration or
    ...
    for candidate in parent.iterdir():
        ...
        marker_count = 0
        if (candidate / SETTINGS_POINTER_FILENAME).is_file():
            marker_count += 1
        if (candidate / LEGACY_SETTINGS_FILENAME).is_file():
            marker_count += 1
        if (candidate / "security").is_dir():
            marker_count += 1
        if (candidate / "service").is_dir():
            marker_count += 1
        if (candidate / "templates").is_dir():
            marker_count += 1
        if (candidate / "locks").is_dir():
            marker_count += 1
...
def get_legacy_config_warning() -> str | None:
    """Return a neutral warning when legacy config directories are found."""
    matches = list_legacy_config_dirs()
    if not matches:
        return None
    joined = ", ".join(str(path) for path in matches[:3])
    if len(matches) > 3:
        joined = f"{joined}, +{len(matches) - 3} more"
    return (
        "Legacy config directory detected. Swirrl will not auto-migrate or "
        f"auto-read it: {joined}"
    )
...
def get_settings_path() -> Path:
    """Return the active ``swirrl.yaml`` path.

    Resolution order:

    1. ``SWIRRL_CONFIG`` environment variable, when set and non-empty.
    2. Nearest existing ``swirrl.yaml`` from the current directory upward,
       stopping before the user home directory or filesystem root.
    3. Configured global settings path stored in ``settings-path.txt``.
    4. User-level ``swirrl.yaml`` in the platform config directory.

    This prevents a plain ``cd`` into a media folder from creating project
    settings implicitly. Project-local settings are opt-in via ``swirrl init``
    or by placing a ``swirrl.yaml`` in the workspace tree.
    """
...
def get_project_presets_dir(base_dir: Path | None = None) -> Path:
    ...
    2. Package-bundled presets shipped inside ``swirrl.presets``.
    ...
    from swirrl.presets import PRESETS_ROOT
