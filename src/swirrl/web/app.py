...
from swirrl.commands.doctor import collect_doctor_payload
from swirrl.core.auth.models import UserRole, UserStore
...
from swirrl.core.webhooks import (
    add_webhook,
    load_webhooks,
    remove_webhook,
    update_webhook,
...
from swirrl.web.modules import (
    ...
    CleanmkvSelectionRequest,
    run_pipeline_plan,
    run_pipeline_inspect,
    create_cleanmkv_selection_preset,
...
# 1) Packaged static assets inside swirrl.web (works for installed wheel/sdist/editable)
...
class TrackerEnabledRequest(BaseModel):
    """Request payload for toggling an upload tracker's enabled flag."""
...
class CheckpointRespondRequest(BaseModel):
    """Request payload for job checkpoint response."""
...
class WorkflowSessionCreateRequest(BaseModel):
    """Request payload for workflow session creation."""
...
class WorkflowDecisionsRequest(BaseModel):
    """Request payload for workflow step decisions upsert."""

    decisions: dict[str, Any]
    source: str = "user"


class WorkflowExecuteStepRequest(BaseModel):
    """Request payload for workflow step execution."""
...
class WorkflowArtifactCreateRequest(BaseModel):
    """Request payload for workflow artifact creation."""
...
def create_app() -> FastAPI:
    """Create FastAPI app exposing Swirrl web endpoints."""
    app = FastAPI(title="Swirrl Web API", version=_resolve_version())
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        allow_methods=["*"],
    ...
    def service_status() -> dict[str, Any]:
        """Return running/stopped status of the Swirrl service process.
        ...
        # PID liveness check.
        #
        # On POSIX, os.kill(pid, 0) is the standard non-destructive probe.
        # On Windows, os.kill has different semantics and may behave
        # inconsistently for liveness probing depending on process/console
        # context, so use Win32 OpenProcess/GetExitCodeProcess instead.
        pid_raw = state.get("pid")
        try:
            pid: int | None = int(pid_raw) if pid_raw is not None else None
    ...
    @app.post("/api/v1/upload/tracker/{tracker_name}/enabled")
    def upload_tracker_set_enabled(
        tracker_name: str, request: TrackerEnabledRequest
    ) -> dict[str, Any]:
        try:
            return {"trackers": set_upload_tracker_enabled(tracker_name, request.enabled)}
        except ValueError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc

    @app.delete("/api/v1/upload/tracker/{tracker_name}")
    def upload_tracker_remove(tracker_name: str) -> dict[str, Any]:
        try:
            return {"trackers": remove_upload_tracker(tracker_name)}
        except ValueError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
    ...
    @app.get("/api/v1/jobs/{job_id}/checkpoint")
    def job_checkpoint(job_id: str) -> dict[str, Any]:
        return get_job_checkpoint(job_id)

    @app.post("/api/v1/jobs/{job_id}/checkpoint/respond")
    def job_checkpoint_respond(job_id: str, request: CheckpointRespondRequest) -> dict[str, Any]:
        try:
            return respond_job_checkpoint(job_id, request.selection_index)
        except ValueError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc

    @app.post("/api/v1/pipeline/inspect")
    def pipeline_inspect(request: PipelineInspectRequest) -> dict[str, Any]:
        try:
            return run_pipeline_inspect(request)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc

    @app.post("/api/v1/cleanmkv/selection-preset")
    def cleanmkv_selection_preset(request: CleanmkvSelectionRequest) -> dict[str, Any]:
        try:
            preset_path = create_cleanmkv_selection_preset(request)
            return {"preset_file": preset_path}
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc

    @app.post("/api/v1/pipeline/plan")
    def pipeline_plan(request: PipelinePlanRequest) -> dict[str, Any]:
        try:
            return run_pipeline_plan(request)
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    def releases_list(limit: int = 20) -> dict[str, Any]:
        if limit < 1 or limit > 200:
            raise HTTPException(status_code=400, detail="limit must be between 1 and 200")
        return {"releases": list_releases(limit=limit)}

    @app.get("/api/v1/releases/{release_id}")
    def releases_get(release_id: str) -> dict[str, Any]:
        release = get_release_by_id(release_id)
        if release is None:
            raise HTTPException(status_code=404, detail="release not found")
        return release

    @app.get("/api/v1/releases/{release_id}/operations")
    def releases_operations(release_id: str) -> dict[str, Any]:
        release = get_release_by_id(release_id)
        if release is None:
            raise HTTPException(status_code=404, detail="release not found")
        return {"operations": list_release_operations(release_id)}

    @app.post("/api/v1/workflow/sessions")
    def workflow_create_session(request: WorkflowSessionCreateRequest) -> dict[str, Any]:
        try:
    ...
    @app.get("/api/v1/workflow/sessions")
    def workflow_list_sessions(
        ...
        if limit < 1 or limit > 200:
            raise HTTPException(status_code=400, detail="limit must be between 1 and 200")
        try:
            return {"sessions": list_workflow_sessions(release_id=release_id, status=status, limit=limit)}
        except ValueError as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc

    @app.get("/api/v1/workflow/sessions/{session_id}")
    def workflow_get_session(session_id: str) -> dict[str, Any]:
        try:
            return get_workflow_session_detail(session_id)
        except LookupError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc

    @app.post("/api/v1/workflow/sessions/{session_id}/steps/{step_key}/decisions")
    def workflow_upsert_decisions(
        session_id: str,
        step_key: str,
        request: WorkflowDecisionsRequest,
    ) -> dict[str, Any]:
        try:
            return upsert_workflow_step_decisions(
                session_id,
                step_key,
                request.decisions,
    ...
    @app.post("/api/v1/workflow/sessions/{session_id}/steps/{step_key}/execute")
    def workflow_execute_step(
        session_id: str,
        step_key: str,
        request: WorkflowExecuteStepRequest,
    ) -> dict[str, Any]:
        try:
    ...
    @app.post("/api/v1/workflow/sessions/{session_id}/steps/{step_key}/skip")
    def workflow_skip_step(session_id: str, step_key: str) -> dict[str, Any]:
        try:
            return skip_workflow_step(session_id, step_key)
        except LookupError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        except RuntimeError as exc:
            raise HTTPException(status_code=409, detail=str(exc)) from exc

    @app.post("/api/v1/workflow/sessions/{session_id}/steps/{step_key}/cancel")
    def workflow_cancel_step(session_id: str, step_key: str) -> dict[str, Any]:
        try:
            return cancel_workflow_step(session_id, step_key)
        except LookupError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        except RuntimeError as exc:
            raise HTTPException(status_code=409, detail=str(exc)) from exc

    @app.post("/api/v1/workflow/sessions/{session_id}/cancel")
    def workflow_cancel_session(session_id: str) -> dict[str, Any]:
        try:
            return cancel_workflow_session(session_id)
        except LookupError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        except RuntimeError as exc:
            raise HTTPException(status_code=409, detail=str(exc)) from exc

    @app.post("/api/v1/workflow/sessions/{session_id}/artifacts")
    def workflow_add_artifact(
        session_id: str,
        request: WorkflowArtifactCreateRequest,
    ) -> dict[str, Any]:
        try:
            return add_workflow_artifact(
                session_id,
                kind=request.kind,
                path=request.path,
    ...
    @app.get("/api/v1/workflow/sessions/{session_id}/artifacts")
    def workflow_get_artifacts(session_id: str) -> dict[str, Any]:
        try:
            return {"artifacts": list_workflow_artifacts(session_id)}
        except LookupError as exc:
            raise HTTPException(status_code=404, detail=str(exc)) from exc
        ...
        try:
            return {
                "rules": add_watch_rule(
                    ...
                    kind_routing=request.kind_routing,
                    presets_by_kind=request.presets_by_kind,
    ...
    def watch_service_start() -> dict[str, Any]:
        ...
        When the embedded watcher is running (service mode via ``swirrl serve``),
    ...
    def webhooks_test_payload(request: WebhookTestPayloadRequest) -> dict[str, Any]:
        from swirrl.core.webhooks import _build_discord_payload, _build_generic_payload, _validate_template  # noqa: PLC0415
        ...
        try:
            ...
            with httpx.Client(timeout=10.0) as client:
                resp = client.post(url, json=payload, headers={"Content-Type": "application/json", "User-Agent": "Swirrl/2.0"})
    ...
    def webhooks_test(webhook_id: str) -> dict[str, Any]:
        from swirrl.core.webhooks import _build_discord_payload, _build_generic_payload  # noqa: PLC0415
        ...
        try:
            import httpx  # noqa: PLC0415
            with httpx.Client(timeout=10.0) as client:
                resp = client.post(wh.url, json=payload, headers={"Content-Type": "application/json", "User-Agent": "Swirrl/2.0"})
                if resp.status_code >= 400:
                    raise HTTPException(status_code=502, detail=f"Webhook returned HTTP {resp.status_code}: {resp.text[:300]}")
        except httpx.RequestError as exc:
            raise HTTPException(status_code=502, detail=f"Request failed: {exc}") from exc
    ...
    if _dist_resolved is not None:
        ...
        async def no_ui() -> JSONResponse:
            ...
            return JSONResponse({
                "message": "Swirrl API is running. Web UI has not been built.",
                "hint": (
