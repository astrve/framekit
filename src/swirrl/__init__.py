"""Swirrl package version.

This value should be kept in sync with the version declared in the project's
``pyproject.toml`` file. It is updated as part of every release to reflect
the current published version. Users and third‑party tools can import this
constant to introspect the installed Swirrl version without having to
parse package metadata at runtime.
"""

# Keep this value in sync with the version declared in pyproject.toml.
__version__ = "2.0.0"
