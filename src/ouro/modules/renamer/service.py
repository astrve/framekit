...
from ouro.core.audit_log import append_audit_event
...
from ouro.modules.renamer.planner import build_rename_plan
from ouro.modules.renamer.profiles import LanguageTagProfile, RenamerProfile
...
class RenamerService:
    ...
    def _apply_case_only_rename(self, source: Path, target: Path) -> None:
        temp_name = f".__ouro_tmp__{uuid.uuid4().hex}{source.suffix}"
    ...
    def run(
        ...
        *,
        ...
        if apply_changes and (strict_conflict_abort or not sys.stdin.isatty()):
            from ouro.ui.console import print_warning
