...
from ouro.core.i18n import temporary_locale, tr
...
from ouro.modules.prez.models import PrezData, PrezField, PrezTrack, PrezTrackGroup
from ouro.modules.prez.theme_loader import get_html_theme_css
...
def _discover_template_names(kind: str, *, fallback: tuple[str, ...]) -> tuple[str, ...]:
    ...
    try:
        root = importlib_resources.files("ouro").joinpath(resource_dir)
        discovered = {
            template_name
...
def _template_environment(kind: str) -> Environment:
    ...
    if kind == "html":
        loader = ChoiceLoader(
            [
                PackageLoader(
                    "ouro", f"templates/prez/{kind}/generated"
                ),  # New generated templates
                PackageLoader("ouro", f"templates/prez/{kind}"),  # Legacy templates
                PackageLoader("ouro", "templates/prez/base"),
            ]
        )
    else:
        loader = ChoiceLoader(
            [
                PackageLoader("ouro", f"templates/prez/{kind}"),  # BBCode templates
                PackageLoader("ouro", "templates/prez/base"),
            ]
        )
