...
from ouro.core.http import HttpAuthError, HttpClient, HttpError
...
from ouro.modules.metadata.health import HealthMonitor
...
class TMDbProvider(MetadataProvider):
    ...
    def _query_params(
        ...
        if not self.config.read_access_token:
            raise ValueError(
                ...
                "in ouro.yaml or export OURO_TMDB_READ_ACCESS_TOKEN."
            )
    ...
    def _request_json(
        ...
        if not self.health_monitor.is_available(self.name):
            ...
            raise OuroMetadataError("TMDb provider is temporarily unavailable")

        # Acquire rate limit token
        if not self.rate_limiter.try_acquire():
            logger.warning("TMDb rate limit exceeded, request throttled")
            self.health_monitor.record_failure(self.name)
            raise OuroMetadataError("TMDb rate limit exceeded")
        ...
        try:
            ...
            raise ValueError(
                ...
                "Check your TMDb read access token with: ouro metadata --status"
            ) from exc
        except HttpError as exc:
            self.health_monitor.record_failure(self.name)
            raise OuroMetadataError(f"TMDb request failed: {exc}") from exc

        if not isinstance(payload, dict):
            self.health_monitor.record_failure(self.name)
            raise OuroMetadataError("TMDb returned an unexpected non-object JSON payload.")
