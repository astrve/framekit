...
from ouro.commands.doctor import collect_doctor_payload
from ouro.core.auth.models import UserRole, UserStore
...
from ouro.core.webhooks import (
    add_webhook,
    load_webhooks,
    remove_webhook,
    update_webhook,
...
# 1) Packaged static assets inside ouro.web (works for installed wheel/sdist/editable)
...
def create_app() -> FastAPI:
    """Create FastAPI app exposing Ouro web endpoints."""
    app = FastAPI(title="Ouro Web API", version=_resolve_version())
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["*"],
        allow_methods=["*"],
    ...
    def service_status() -> dict[str, Any]:
        """Return running/stopped status of the Ouro service process.
    ...
    def watch_service_start() -> dict[str, Any]:
        ...
        When the embedded watcher is running (service mode via ``ouro serve``),
    ...
    def webhooks_test_payload(request: WebhookTestPayloadRequest) -> dict[str, Any]:
        from ouro.core.webhooks import _build_discord_payload, _build_generic_payload, _validate_template  # noqa: PLC0415
        ...
        try:
            ...
            with httpx.Client(timeout=10.0) as client:
                resp = client.post(url, json=payload, headers={"Content-Type": "application/json", "User-Agent": "Ouro/2.0"})
    ...
    def webhooks_test(webhook_id: str) -> dict[str, Any]:
        from ouro.core.webhooks import _build_discord_payload, _build_generic_payload  # noqa: PLC0415
        ...
        try:
            import httpx  # noqa: PLC0415
            with httpx.Client(timeout=10.0) as client:
                resp = client.post(wh.url, json=payload, headers={"Content-Type": "application/json", "User-Agent": "Ouro/2.0"})
                if resp.status_code >= 400:
                    raise HTTPException(status_code=502, detail=f"Webhook returned HTTP {resp.status_code}: {resp.text[:300]}")
        except httpx.RequestError as exc:
            raise HTTPException(status_code=502, detail=f"Request failed: {exc}") from exc
    ...
    if _dist_resolved is not None:
        ...
        async def no_ui() -> JSONResponse:
            ...
            return JSONResponse({
                "message": "Ouro API is running. Web UI has not been built.",
                "hint": (
