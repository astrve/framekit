"""Linux user-service management for Ouro via systemd --user.

Provides helpers for installing, uninstalling, starting, stopping, and
querying a Ouro service unit in ``~/.config/systemd/user``.
...
_UNIT_NAME = "ouro.service"
_DESCRIPTION = "Ouro Web Service"
...
def _build_exec_start(host: str, port: int) -> str:
    """Return ExecStart command using ``ouro serve``."""
    ouro_exe = shutil.which("ouro")
    host_arg = shlex.quote(host)
    if ouro_exe:
        return (
            f"{shlex.quote(ouro_exe)} serve "
            f"--host {host_arg} --port {int(port)}"
        )
    return (
        f"{shlex.quote(sys.executable)} -m ouro serve "
        f"--host {host_arg} --port {int(port)}"
    )
...
def _is_missing_unit_message(message: str) -> bool:
    ...
    patterns = (
        ...
        "not-found",
        "no such file",
        "unit ouro.service could not be found",
        "unit ouro.service not found",
    )
...
def install_systemd_user(
    ...
    """Install ouro user-service unit and enable at login."""
    guard = _linux_only()
...
def uninstall_systemd_user(unit_dir: Path | None = None) -> tuple[bool, str]:
    """Disable and remove ouro user-service unit."""
    guard = _linux_only()
    if guard is not None:
        return guard
...
def query_systemd_status() -> dict[str, str]:
    """Return status from ``systemctl --user show ouro.service``."""
    guard = _linux_only()
    if guard is not None:
...
def journal_logs(lines: int = 50, follow: bool = False) -> tuple[bool, str]:
    """Return/tail journal logs for ouro user unit."""
    guard = _linux_only()
    if guard is not None:
        return guard
