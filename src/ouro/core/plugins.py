...
Ouro exposes a public extension point named ``ouro.modules``. Any
installed Python distribution that declares an entry-point under this group
can contribute a new CLI subcommand without forking Ouro:

```toml
# pyproject.toml of a third-party package, e.g. ``ouro-plugin-anilist``
[project.entry-points."ouro.modules"]
anilist = "ouro_plugin_anilist:register"
```
...
ENTRY_POINT_GROUP = "ouro.modules"
...
def iter_plugin_entry_points() -> list[importlib_metadata.EntryPoint]:
    """Return every entry-point declared under ``ouro.modules``.
...
def load_plugins(cli: click.Group) -> list[str]:
    """Discover and register every ``ouro.modules`` plugin on ``cli``.
    ...
    Args:
        cli: The Click root group exposed by ``ouro.commands.main``.
    ...
    for entry_point in iter_plugin_entry_points():
        ...
        if not is_allowed:
            logger.info(
                f"Skipping ouro plugin {name!r} from distribution "
        ...
        try:
            ...
            logger.warning(
                f"Skipping ouro plugin {name!r}: failed to import "
        ...
        if not callable(register):
            logger.warning(
                f"Skipping ouro plugin {name!r}: entry-point target "
        ...
        try:
            ...
            logger.warning(
                f"Skipping ouro plugin {name!r}: register() raised "
                f"({exc.__class__.__name__}: {exc})"
            )
            continue

        loaded.append(name)
        logger.debug(f"Loaded ouro plugin: {name}")
...
def list_installed_plugins() -> list[dict[str, Any]]:
    ...
    Useful for ``ouro doctor`` or a future ``ouro plugins`` listing.
