"""Safe subprocess wrapper used by every external-tool invocation in Ouro.
...
def resolve_tool(name: str) -> str:
    ...
    if resolved is None:
        raise MissingToolError(
            ...
            "path in ouro.yaml under `tools:`, then re-run "
            "`ouro doctor --check-tools` to verify.",
            tool=name,
            argv=(name,),
        )
    return resolved
