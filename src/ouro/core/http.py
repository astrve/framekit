...
from ouro.core.exceptions import OuroHttpError
...
def _ouro_user_agent() -> str:
    """Return ``Ouro/<version>`` using the installed package metadata."""
    try:
        ...
        return "Ouro/unknown"
    try:
        return f"Ouro/{pkg_version('ouro-auto')}"
    except PackageNotFoundError:
        return "Ouro/unknown"


DEFAULT_USER_AGENT = _ouro_user_agent()
...
class HttpClientConfig:
    ...
    user_agent: str = field(default_factory=_ouro_user_agent)
...
class HttpError(OuroHttpError):
    """HttpError raised on ouro failures."""
...
def _classify_connect_error(exc: httpx.ConnectError, url: str) -> HttpNetworkError:
    """Translate ``httpx.ConnectError`` into the right Ouro exception."""
