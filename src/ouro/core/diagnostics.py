...
LOG_ENV_VAR = "OURO_LOG_FILE"
DEBUG_ENV_VAR = "OURO_DEBUG"
...
def _collect_log_backups(log_dir: Path) -> list[Path]:
    log_files: list[Path] = []
    for pattern in ("ouro.log.*", "ouro.log.*.gz"):
...
def default_log_file() -> Path:
    ...
    return get_cache_dir() / "logs" / "ouro.log"
...
def _module_label_from_record_name(record_name: str) -> str:
    ...
    if second == "__main__":
        return "ouro"
    if second == "core":
        return "ouro" if len(parts) > 2 and parts[2] == "diagnostics" else "core"
    if second == "ui":
        return "ui"
...
def cleanup_old_logs() -> dict[str, Any]:
    ...
    before_count = len(list(log_dir.glob("ouro.log.*")))
    before_count += len(list(log_dir.glob("ouro.log.*.gz")))

    # Perform cleanup
    _cleanup_old_logs(log_dir, _STATE.retention_days, _STATE.max_backups)

    # Count files after cleanup
    after_count = len(list(log_dir.glob("ouro.log.*")))
    after_count += len(list(log_dir.glob("ouro.log.*.gz")))
...
def get_log_rotation_status() -> dict[str, Any]:
    ...
    if log_file.parent.exists():
        backup_count = len(list(log_file.parent.glob("ouro.log.*")))
        backup_count += len(list(log_file.parent.glob("ouro.log.*.gz")))
...
# entry point in ``ouro/__main__.py``) invoke it explicitly once the
