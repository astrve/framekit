...
Compares the installed ``ouro-auto`` version against the latest release on
PyPI no more than once per day, and prints a subtle upgrade banner when a
newer version is available.

The check is intentionally lightweight:

* Cached for 24 h in ``~/.cache/ouro/update_check.json`` (per-user).
* Disabled via ``OURO_DISABLE_UPDATE_CHECK=1`` or by the
...
PYPI_URL = "https://pypi.org/pypi/ouro-auto/json"
CACHE_TTL_SECONDS = 24 * 60 * 60
NETWORK_TIMEOUT_SECONDS = 2.0
DISABLE_ENV_VAR = "OURO_DISABLE_UPDATE_CHECK"
...
def _installed_version() -> str | None:
    """Return the installed ``ouro-auto`` version, or ``None`` when unknown."""
    for dist_name in ("ouro-auto", "ouro"):
        try:
...
def show_update_banner_if_available() -> None:
    ...
    try:
        ...
        console.print(
            f"[yellow]▲ Ouro {info.latest} is available "
            f"(installed {info.installed}). Upgrade with:[/yellow] "
            f"[cyan]pip install -U ouro-auto[/cyan]"
        )
    except Exception:
        # Fallback to a plain stderr line if rich is somehow unavailable.
        sys.stderr.write(
            f"Ouro {info.latest} available (installed {info.installed}). "
            f"Run: pip install -U ouro-auto\n"
        )
