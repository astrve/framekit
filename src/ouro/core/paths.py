...
SETTINGS_FILENAME = "ouro.yaml"
LEGACY_SETTINGS_FILENAME = "settings.json"
SETTINGS_POINTER_FILENAME = "settings-path.txt"
ENV_CONFIG_PATH = "OURO_CONFIG"
ENV_CONFIG_DIR = "OURO_CONFIG_DIR"
ENV_CACHE_DIR = "OURO_CACHE_DIR"
...
def get_settings_path() -> Path:
    """Return the active ``ouro.yaml`` path.

    Resolution order:

    1. ``OURO_CONFIG`` environment variable, when set and non-empty.
    2. Nearest existing ``ouro.yaml`` from the current directory upward,
       stopping before the user home directory or filesystem root.
    3. Configured global settings path stored in ``settings-path.txt``.
    4. User-level ``ouro.yaml`` in the platform config directory.

    This prevents a plain ``cd`` into a media folder from creating project
    settings implicitly. Project-local settings are opt-in via ``ouro init``
    or by placing a ``ouro.yaml`` in the workspace tree.
    """
...
def get_project_presets_dir(base_dir: Path | None = None) -> Path:
    ...
    2. Package-bundled presets shipped inside ``ouro.presets``.
    ...
    from ouro.presets import PRESETS_ROOT
