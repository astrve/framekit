...
def _force_utf8_stdio() -> None:
    ...
    On Windows, when the ouro binary is launched from a non-Unicode
...
def _apply_dry_run(argv: list[str]) -> list[str]:
    """Promote a leading ``--dry-run`` to ``OURO_DRY_RUN=1``.

    When ``--dry-run`` appears *before* the subcommand name it is a global
    shorthand (``ouro --dry-run pipeline …``).  We strip it from argv so
...
def _apply_auto_yes(argv: list[str]) -> list[str]:
    """Promote a leading ``--yes`` / ``-y`` to ``OURO_AUTO_YES=1``.

    When the flag is present at the root, every confirmation prompt
    surfaced through :func:`ouro.core.destructive.confirm_destructive`
...
from ouro.core.diagnostics import (  # noqa: E402
...
from ouro.core.exceptions import OuroError  # noqa: E402
from ouro.core.i18n import set_locale, tr  # noqa: E402
from ouro.core.settings import SettingsStore  # noqa: E402
from ouro.ui.console import console, print_error  # noqa: E402
...
def _print_suggestions_for(exc: BaseException) -> None:
    ...
    Uses the explicit ``suggestions`` tuple on :class:`OuroError` when
    present, otherwise falls back to heuristic matching. Never raises.
    """
    try:
        from ouro.core.error_suggestions import derive_suggestions
...
def _friendly_click_message(exc: click.ClickException) -> str:
    ...
    hints = {
        ...
        "--html-template": "You need to provide an HTML Prez template. Use ouro prez --list-templates first if needed.",
        "--bbcode-template": "You need to provide a BBCode Prez template. Use ouro prez --list-templates first if needed.",
...
def _resolve_ouro_version() -> str:
    try:
        ...
        from ouro import __version__ as fallback_version
...
def _print_version_and_exit() -> int:
    sys.stdout.write(f"ouro, version {_resolve_ouro_version()}\n")
    return 0
...
def _run_update_check_background() -> None:
    ...
    try:
        from ouro.core.update_check import start_background_update_check
...
def _run_cli(args: list[str]) -> int:
    ...
    from ouro.commands.setup import maybe_offer_first_time_setup
...
def main() -> int:
    ...
    try:
        log_event("INFO", "Ouro command started", argv=args)
        _load_locale_from_settings()
        _run_update_check_background()
        exit_code = _run_cli(args)
        log_event("INFO", "Ouro command completed", exit_code=exit_code)
        return exit_code
        ...
        print_error(
            tr(
                exc.message_key or "runtime.ouro_error",
                default=str(exc),
                message=str(exc),
                **exc.context,
            )
