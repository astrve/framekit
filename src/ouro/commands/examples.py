"""``ouro examples`` — print common command recipes in the active locale.
...
main use cases. Translations live in ``ouro.locales`` so the recipes
...
_RECIPES: tuple[_Recipe, ...] = (
    _Recipe(
        ...
        command="ouro pipeline /path/to/Release.2024.1080p.WEB-DL.x264-GROUP",
    ),
    _Recipe(
        ...
        command="ouro batch /path/to/releases --auto --pipeline-preset default",
    ),
    _Recipe(
        ...
        command="ouro metadata /path/to/Release -y",
    ),
    _Recipe(
        ...
        command="ouro cleanmkv /path/to/Release.mkv",
    ),
    _Recipe(
        ...
        command="ouro nfo /path/to/Release -m",
    ),
    _Recipe(
        ...
        command="ouro torrent /path/to/Release --announce https://tracker/announce",
    ),
    _Recipe(
        ...
        command="ouro prez /path/to/Release --html-template aurora --bbcode-template classic",
    ),
    _Recipe(
    ...
    _Recipe(
        ...
        command="ouro pipeline /path/to/Release --ren --nfo --tor",
    ),
    _Recipe(
        ...
        command="ouro language nfo fr\nouro language metadata fr-FR",
    ),
    _Recipe(
        ...
        command="ouro profile create anime --wizard",
    ),
    _Recipe(
...
@click.group(
    ...
    help=tr(
        "cli.examples.help",
        default=(
            "Print ready-to-copy command recipes covering the main Ouro "
            "workflows.\n\n"
            "Subcommands:\n"
    ...
    console.print(
        Panel(
            tr(
                "examples.intro",
                default=(
                    "Common Ouro recipes. Copy a line, adjust the paths to "
                    "your release, and run."
                ),
            ),
            title=tr("examples.title", default="Ouro recipes"),
            border_style="white",
            box=box.HEAVY,
            expand=True,
        )
...
def examples_alias_command() -> None:
    ...
    from ouro.commands.alias import _get_alias_manager
