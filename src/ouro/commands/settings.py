...
from ouro.core.paths import get_intelligent_cache_dir
...
def _settings_tables(settings: dict[str, Any]) -> list[Table]:
    ...
    overview = Table(
        title=tr("settings.overview", default="Ouro overview"),
        box=box.HEAVY,
        expand=True,
    )
...
def run_settings_show(*, json_output: bool = False) -> int:
    ...
    """
    from ouro.core.json_output import emit_json, json_envelope
    from ouro.core.settings import redact_settings
...
def run_security_migrate() -> int:
    ...
    try:
        ...
        if not settings_obj.is_security_enabled():
            print_warning("Security is disabled. Enable it first in ouro.yaml")
            return 1
...
def run_security_reset_vault() -> int:
    ...
    if not settings_obj.is_security_enabled():
        print_warning(
            ...
            "Enable it first with: ouro settings security toggle --enable"
        )
        return 1
    ...
    console.print("  [cyan]ouro settings security set-token[/cyan]")
    console.print("  [cyan]ouro settings security set-announces[/cyan]")
    return 0
...
def _hint_reset_vault() -> None:
    ...
    console.print(
        ...
        "[cyan]ouro settings security reset-vault[/cyan]"
    )
    console.print(
        "  • Or disable security entirely: [cyan]ouro settings security toggle --disable[/cyan]"
    )
...
def run_security_toggle(enable: bool | None) -> int:
    ...
    from ouro.core.settings import Settings, SettingsStore
    ...
    try:
        ...
        if enable:
            ...
            console.print(
                "[cyan]Tip: Use 'ouro settings security migrate' to move existing sensitive data to the vault[/cyan]"
            )
        else:
...
@click.group(
    ...
    help=tr(
        "cli.settings.help",
        default=(
            "Inspect Ouro configuration, cache and security.\n\n"
            "Examples:\n"
            "  ouro settings                     Show all settings\n"
            "  ouro settings reset general.locale\n"
            "  ouro settings cache stats\n"
            "  ouro settings security status\n\n"
            "Subcommands:\n"
...
def settings_command(ctx: click.Context, json_output: bool) -> int | None:
    """Display or update the active Ouro settings.
...
def settings_doctor_command(json_output: bool) -> int:
    ...
    from ouro.commands.doctor import run_doctor_command

    return run_doctor_command(json_output=json_output)


from ouro.commands.benchmark import benchmark_command  # noqa: E402
