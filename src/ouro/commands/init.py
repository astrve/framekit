"""Non-interactive ``ouro init`` command.

Drops a ``ouro.yaml`` (and optionally a ``Presets/`` skeleton) in the
current working directory. Intended for CI/CD, scripting and headless
deployments — the inverse of ``ouro setup``, which is interactive.
"""
...
from ouro.core.settings import DEFAULT_SETTINGS, SettingsStore
        ...
        print_info(
            tr(
                ...
                default=(
                    ...
                    "security.enabled and run ``ouro settings security "
    ...
    print_warning(
        tr(
            ...
            default=(
                ...
                "skeleton. Run ``ouro settings security enable`` to "
...
def _print_init_next_steps() -> None:
    ...
    print_info(
        tr(
            ...
            default=(
                "Next steps:\n"
                "  ouro setup    # interactive configuration wizard\n"
                "  ouro doctor   # validate tools and configuration\n"
...
@click.command(
    ...
    help=tr(
        "cli.init.help",
        default=(
            "Initialise a Ouro workspace non-interactively.\n\n"
            "Drops a ``ouro.yaml`` in the current directory and (unless "
            "``--minimal`` is passed) creates an empty ``Presets/`` tree. "
            "This is the headless counterpart of ``ouro setup`` — it is "
            "safe to run from CI, Docker entrypoints, or shell scripts.\n\n"
            "Examples:\n"
            "  ouro init                            # create ouro.yaml + Presets/\n"
            "  ouro init --force                    # overwrite existing files\n"
            "  ouro init --minimal                  # no Presets, blank vault config\n"
            "  ouro init --token eyJ.abc.xyz        # pre-fill TMDb v4 read token\n"
            "\n"
            "For an interactive walk-through use ``ouro setup`` instead."
        ),
    ),
)
@click.option(
    ...
    help=tr(
        "cli.init.force",
        default="Overwrite ouro.yaml if it already exists.",
    ),
)
@click.option(
    "--minimal",
...
def init_command(
    ...
    """Drop a project-local ouro.yaml (and presets) in ``target_dir``."""
