...
from ouro.commands.cleanmkv import run_cleanmkv_command
from ouro.commands.nfo import run_nfo_command
from ouro.commands.renamer import run_renamer_command
from ouro.commands.torrent import run_torrent_command
from ouro.core.i18n import tr
from ouro.core.models.nfo import ReleaseNfoData
from ouro.core.naming import torrent_name_from_payload
...
from ouro.modules.prez.service import PREZ_PRESETS, PrezBuildOptions, PrezService
...
def _prez_step(
    ...
    """
    if context is None or settings is None:
        from ouro.commands.prez import run_prez_command
...
def _resolve_screenshot_pipeline_config(settings: dict) -> tuple[object, str, str]:
    from ouro.core.models.screenshot import ScreenshotConfig
...
def _render_screenshot_nfo_block(screenshots: Sequence[Path]) -> str:
    lines = ["", "<!-- Ouro Screenshots -->", "<screenshots>"]
...
def _render_screenshot_html_block(screenshots: Sequence[Path]) -> str:
    lines = [
        "",
        "<section class=\"ouro-screenshots\">",
        "  <h2>Screenshots</h2>",
        "  <div class=\"ouro-screenshots-grid\">",
    ]
    for path in screenshots:
        src = path.as_posix()
...
def _screenshot_step(
    ...
    from ouro.modules.screenshot.service import ScreenshotService
...
def _rename_parent_step(root: Path, settings: dict | None = None, *, dry_run: bool = False) -> int:
    ...
    from ouro.commands.tools import _derive_name_from_payload, _derive_name_from_renamer, _rename_folder
...
def _resolve_prez_templates_for_pipeline(
    *,
    ...
    from ouro.commands.prez import (
...
def _encoder_step(work_folder: Path, settings: dict) -> int:
    ...
    Returns:
        Exit code (0 for success, non-zero for failure)
    """
    from ouro.modules.encoder import EncoderService, PresetLoader
...
def _resolve_pipeline_encoder_preset(preset_name: str, loader):
    from ouro.modules.encoder import PresetLoader
...
def _resolve_pipeline_upload_description(
    *,
    ...
    description = "Release created with Ouro"
...
def _pipeline_best_effort_dupe_check(tracker_name: str, metadata: TorrentMetadata) -> None:
    ...
    try:
        ...
        from ouro.modules.upload.service import UploadService as _US

        first_config = _US()._load_tracker_config(tracker_name)  # pyright: ignore[reportPrivateUsage]
        from ouro.modules.upload.adapters.gazelle import GazelleAdapter
        from ouro.modules.upload.adapters.unit3d import UNIT3DAdapter
...
def _upload_step(
    ...
    """
    from ouro.modules.upload.image_host import ImageHostService, inject_images_into_bbcode
    from ouro.modules.upload.metadata_extractor import NFOData, NFOParser, ReleaseParser
