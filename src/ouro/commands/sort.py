"""``ouro sort`` — sort release folders into categorized subdirectories.
    ...
    *,
    ...
    if not path:
        print_error(
            tr(
                "sort.error.no_path",
                default="Please specify the folder to sort: ouro sort <path>",
            )
        )
        return 1
...
@click.command(
    ...
    help=tr(
        ...
        default=(
            ...
            "Quick examples:\n"
            "  ouro sort -s /path/to/downloads               # Preview + confirm\n"
            "  ouro sort -s /path/to/downloads -d             # Dry-run only\n"
            "  ouro sort -s /downloads -t /nas/media          # Sort into separate target\n"
            "  ouro sort -s /path/to/downloads --copy         # Copy instead of move\n"
            "  ouro sort -s /path/to/downloads -R             # Sort + normalize folder names"
        ),
    ),
)
