...
from ouro.modules.encoder import EncoderService, EncoderValidator, PresetLoader
...
def _print_encoder_init_error(exc: Exception) -> None:
    ...
    console.print("     ouro settings set encoder.ffmpeg_path <path-to-ffmpeg>")
    console.print("  3. Run 'ouro encode check' to verify installation")
...
def encode_command(
    ...
    Examples:
        ouro encode run input.mkv --preset films
        ouro encode run input.mkv --preset-file custom.yaml --output output.mkv
        ouro encode run folder/ --preset series --recursive --output-dir encoded/
    """
    store = SettingsStore()
    settings = store.load()
    ...
    try:
        ...
        print_info("Use 'ouro encode list-presets' to see available presets")
        return 1
    except ValueError:
...
def create_preset_command(output: str, preset_type: str) -> int:
    ...
    try:
        ...
        print_info(f"  ouro encode run input.mkv --preset-file {output_path}")
        return 0
    except Exception as e:
