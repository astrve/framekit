...
def _handle_missing_credentials(json_output: bool) -> int:
    ...
    print_info(tr("metadata.info.run_setup", default="Run: ouro setup"))
    print_info(tr("metadata.info.check_status", default="Or check status with: ouro md -s"))
    return 1
...
def run_metadata_command(
    *,
    ...
    """Drive the ``ouro metadata`` command end-to-end.
    ...
    resolved metadata) — see :func:`ouro.core.json_output.json_envelope`.
    """
    store = SettingsStore()
    settings = store.load()
...
@click.command(
    ...
    help=tr(
        ...
        default=(
            ...
            "Quick examples:\n"
            "  ouro metadata <folder>                    # Interactive metadata search\n"
            "  ouro meta <folder> -y                     # Auto-accept top match\n"
            "  ouro md <file.mkv>                        # Resolve single episode\n"
            "  ouro metadata --status                    # Check configuration\n"
            "  ouro meta --token                         # Set TMDb API token\n\n"
            "Setup:\n"
            "  1. Get a TMDb Read Access Token from https://www.themoviedb.org/settings/api\n"
            "  2. Run: ouro metadata --token\n"
            "  3. Paste your token when prompted\n"
            "  4. Test with: ouro meta <folder>\n\n"
            "Features:\n"
