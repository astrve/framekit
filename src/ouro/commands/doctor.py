...
from ouro.commands.benchmark import benchmark_command
from ouro.core.diagnostics import diagnostics_summary
...
from ouro.modules.metadata.config import resolve_metadata_config
...
from ouro.modules.nfo.templates import list_builtin_templates
...
from ouro.ui.console import console, print_json
...
def _network_checks(settings: dict) -> list[DoctorCheck]:
    ...
    endpoints = (
        ...
        ("PyPI", "https://pypi.org/pypi/ouro-auto/json"),
    )
...
def _vault_checks(settings: dict) -> list[DoctorCheck]:
    ...
    for label, path in (
        ...
        if not Path(path).exists():
            ...
            continue
        try:
            from ouro.core.security.permissions import verify_secure_permissions
        ...
        checks.append(
            DoctorCheck(
                ...
                if secure
                    ...
                    default="loose permissions (run: ouro settings security repair)",
                ),
            )
        )
...
@click.group(
    ...
    help=tr(
        ...
        default=(
            ...
            "The doctor command performs health checks on your Ouro installation, verifying "
            "external tools, settings, metadata configuration, templates, and more.\n\n"
            "Quick examples:\n"
            "  ouro doctor                               # Run all checks\n"
            "  ouro diag -j                              # Output as JSON\n"
            "  ouro doc                                  # Quick health check\n"
            "  ouro doctor benchmark                     # Run performance benchmarks\n\n"
            "Checks performed:\n"
