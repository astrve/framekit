...
from ouro.core.cli_helpers import print_fatal, print_warning
from ouro.core.paths import get_config_dir, get_pipeline_presets_dir
from ouro.core.settings import SettingsStore
from ouro.modules.watch import WatchConfig, WatcherService
from ouro.modules.watch.service import (
...
def _validate_enabled_folders_for_start(enabled_folders: list[dict]) -> bool:
    ...
    _show_available_presets()
    console.print(
        "Fix with: [cyan]ouro watch remove[/cyan] then [cyan]ouro watch add[/cyan]"
    )
    return False
...
def setup_command():
    ...
    notifications. Saves the result to ouro.yaml.
    """
    _reset_timeline()
    ...
    try:
        ...
        if folders:
            console.print("Start with: [cyan]ouro watch start[/cyan]")
        else:
            console.print("Add folders with: [cyan]ouro watch add[/cyan]")

    except KeyboardInterrupt:
        console.print("\n[yellow]Setup cancelled[/yellow]")
    except Exception as e:
        print_fatal(str(e), "Check that ouro.yaml is writable")
...
def remove_command():
    ...
    try:
        ...
        if not folders:
            ...
            console.print("Add one with: [cyan]ouro watch add[/cyan]")
            return
...
def start_command(start_all: bool, no_status_updates: bool, status_interval: int):
    ...
    \b
    Examples:
      ouro watch start           # choose folders interactively
      ouro watch start --all     # start all enabled folders
    """
    try:
        ...
        if not enabled_folders:
            ...
            console.print("Add one with: [cyan]ouro watch add[/cyan]")
            console.print("Or enable folders with: [cyan]ouro watch list[/cyan]")
            return
        ...
        print_fatal(str(e), "Check your configuration with: ouro watch test")
...
def status_command(json_output: bool):
    ...
    try:
        ...
        if not folders:
            ...
            console.print("Run: [cyan]ouro watch add[/cyan]")
            return
        ...
        print_fatal(str(e), "Verify watch configuration in ouro.yaml")
...
def stop_command(timeout: float) -> None:
    ...
    Looks for ``.ouro_watch.pid`` next to the active ouro.yaml and
...
def test_command(folder: Path | None, preset: str | None):
    ...
    Examples:
      ouro watch test
      ouro watch test --folder D:\\downloads --preset multi_fr
    """
    try:
        _store, settings, folders = _load_watch_data()
        ...
        if not folders:
            print_fatal(
                "No watch configuration found", "Run 'ouro watch add' or 'ouro watch setup'"
            )
            return
        ...
        if all_valid:
            ...
            print_fatal("Configuration has errors - run 'ouro watch setup' to fix")
