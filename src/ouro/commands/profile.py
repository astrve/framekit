...
from ouro.core.settings.store import SettingsStore
from ouro.ui.click_helper import click
from ouro.ui.console import console, print_error, print_info, print_success
from ouro.ui.unified_selector import SelectorOption, confirm_choice, text_input
...
def profile_list() -> None:
    ...
    if not profiles:
        print_info("No profiles found. Create one with: ouro profile create <name>")
        return
...
def profile_use(name: str | None) -> None:
    ...
    profiles = list_profiles()
    if not profiles:
        print_error("No profiles available. Create one first: ouro profile create <name>")
        return
...
def profile_create(name: str, description: str, from_current: bool, wizard: bool) -> None:
    ...
    Examples:
        ouro profile create anime -d "Anime workflow with subs"
        ouro profile create movie-4k --from-current
        ouro profile create my-preset --wizard
    """
    if from_current:
        store = SettingsStore()
        current = store.load()
