...
from ouro.commands.pipeline_preview import print_pipeline_report
...
from ouro.ui.console import print_error, print_info, print_success, print_warning
...
if TYPE_CHECKING:
    from ouro.commands.pipeline import PipelineStep
...
def _pipeline_output_folder(work_folder: Path, root: Path | None = None) -> Path:
    ...
    """
    from ouro.commands.pipeline import _release_artifacts_folder
...
def _auto_enable_seedbox(selected_modules: set[str], settings: dict) -> set[str]:
    ...
    try:
        from ouro.commands.seedbox import seedbox_ready_for_pipeline
