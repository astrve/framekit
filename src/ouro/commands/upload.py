...
from ouro.core.cli_helpers import print_fatal
from ouro.core.paths import get_config_dir
from ouro.core.settings import SettingsStore, redact_settings
from ouro.modules.upload import DiscoveryService, TorrentMetadata, UploadService
from ouro.ui.console import console
from ouro.ui.timeline import reset_timeline, show_step
...
# ── Timeline helpers (shared from ouro.ui.timeline) ──────────────────────
...
def assistant_command(
    ...
    env_name = token_env or text_input(
        ...
        default=f"OURO_TRACKER_{_slugify_tracker_name(name).upper().replace('-', '_')}_TOKEN",
    )
...
def _infer_release_display_name(torrent_file: Path) -> str:
    ...
    try:
        from ouro.modules.upload.metadata_extractor import ReleaseParser
...
def _apply_upload_preset(
    ...
    *,
    ...
    if not preset:
        return metadata
    from ouro.modules.upload.presets import PresetManager
...
def _setup_image_host(steps: list[str]) -> _ImageHostSettings:
    ...
    from ouro.modules.upload.image_host import ImageHostService
...
def _setup_save_configuration(
    *,
    ...
    from ouro.modules.upload.models import DiscoveryResult
...
def _print_setup_summary(
    ...
    console.print(
        "\nTest with: [cyan]ouro upload test --tracker " + credentials.tracker_name + "[/cyan]"
    )
...
def _resolve_run_context(
    ...
    if not trackers:
        print_fatal("No trackers configured", "Run: ouro upload setup")
        return None
...
def _try_auto_tmdb_id(torrent_file: Path) -> int | None:
    ...
    try:
        ...
        from ouro.core.settings import SettingsStore
        from ouro.modules.metadata.factory import build_metadata_provider
        from ouro.modules.upload.metadata_extractor import ReleaseParser
...
def setup_command():
    ...
    testing the connection, and saving to ouro.yaml.
    """
    ...
    try:
        ...
        _show_step("Step 5 - Save", "Saving configuration to ouro.yaml.", "Save", steps)
...
def remove_command():
    ...
    try:
        ...
        if not trackers:
            ...
            console.print("Add one with: [cyan]ouro upload setup[/cyan]")
            return
...
def list_trackers_command():
    ...
    try:
        ...
        if not trackers:
            console.print("[yellow]No trackers configured[/yellow]")
            console.print("Run: [cyan]ouro upload setup[/cyan]")
            return
        ...
        print_fatal(str(e), "Check ouro.yaml for tracker configuration")
...
@click.option("--save/--no-save", default=True, help="Save to ouro.yaml")
def discover_command(url: str, api_key: str, name: str, tracker_type: str | None, save: bool):
    ...
    \b
    Examples:
      ouro upload discover --url https://tracker.com --api-key KEY --name mytracker
      ouro upload discover --url https://tracker.com --api-key KEY --name t --type unit3d
    """
    ...
    try:
        ...
        if save:
            if discovery.save_discovery(result, name, api_key):
                console.print("\n[green]Configuration saved to ouro.yaml[/green]")
            else:
...
def run_command(
    ...
    \b
    Examples:
      ouro upload run movie.torrent --tracker bhd --name "Film 2024" \\
          --description "..." --category Films --type 1080p --resolution 1920x1080 --tmdb-id 12345

      ouro upload run movie.torrent   # fully interactive
      ouro upload run "E:\\Release\\MyFolder" --tracker my-tracker --dry-run
    """
    try:
...
def stats_command(days: int):
    ...
    from ouro.modules.upload.stats import UploadStats
...
def search_command(query: str):
    ...
    from ouro.modules.upload.dupe_checker import DupeChecker
    from ouro.modules.upload.models import TorrentMetadata
    from ouro.modules.upload.service import UploadService

    service = UploadService()
    from ouro.core.settings_singleton import get_settings_data
...
def preset_add():
    ...
    from ouro.modules.upload.presets import PresetManager, UploadPreset
...
def queue_list():
    ...
    from ouro.modules.upload.queue import UploadQueue
