"""``framekit examples`` — print common command recipes in the active locale.
...
main use cases. Translations live in ``framekit.locales`` so the recipes
...
_RECIPES: tuple[_Recipe, ...] = (
    _Recipe(
        ...
        command="framekit pipeline /path/to/Release.2024.1080p.WEB-DL.x264-GROUP",
    ),
    _Recipe(
        ...
        command="framekit batch /path/to/releases --auto --pipeline-preset default",
    ),
    _Recipe(
        ...
        command="framekit metadata /path/to/Release -y",
    ),
    _Recipe(
        ...
        command="framekit cleanmkv /path/to/Release.mkv",
    ),
    _Recipe(
        ...
        command="framekit nfo /path/to/Release -m",
    ),
    _Recipe(
        ...
        command="framekit torrent /path/to/Release --announce https://tracker/announce",
    ),
    _Recipe(
        ...
        command="framekit prez /path/to/Release --html-template aurora --bbcode-template classic",
    ),
    _Recipe(
        ...
        description_default=(
            "Once configured via ``framekit watch setup``, this runs in the "
            "foreground and triggers the pipeline whenever a new release "
            "lands in the watched folder."
        ),
        command="framekit watch start --all",
    ),
    _Recipe(
        ...
        command="framekit pipeline /path/to/Release --ren --nfo --tor",
    ),
    _Recipe(
        ...
        command="framekit language nfo fr\nframekit language metadata fr-FR",
    ),
    _Recipe(
        ...
        command="framekit profile create anime --wizard",
    ),
    _Recipe(
        ...
        command="framekit settings doctor",
    ),
    _Recipe(
...
@click.group(
    ...
    help=tr(
        "cli.examples.help",
        default=(
            "Print ready-to-copy command recipes covering the main Framekit "
            "workflows.\n\n"
            "Subcommands:\n"
    ...
    console.print(
        Panel(
            tr(
                "examples.intro",
                default=(
                    "Common Framekit recipes. Copy a line, adjust the paths to "
                    "your release, and run."
                ),
            ),
            title=tr("examples.title", default="Framekit recipes"),
            border_style="white",
            box=box.HEAVY,
            expand=True,
        )
...
def examples_alias_command() -> None:
    ...
    from framekit.commands.alias import _get_alias_manager
