"""Non-interactive ``framekit init`` command.

Drops a ``framekit.yaml`` (and optionally a ``Presets/`` skeleton) in the
current working directory. Intended for CI/CD, scripting and headless
deployments — the inverse of ``framekit setup``, which is interactive.
"""
...
from framekit.core.settings import DEFAULT_SETTINGS, SettingsStore
        ...
        print_info(
            tr(
                ...
                default=(
                    ...
                    "security.enabled and run ``framekit settings security "
    ...
    print_warning(
        tr(
            ...
            default=(
                ...
                "skeleton. Run ``framekit settings security enable`` to "
...
def _print_init_next_steps() -> None:
    ...
    print_info(
        tr(
            ...
            default=(
                "Next steps:\n"
                "  framekit setup    # interactive configuration wizard\n"
                "  framekit doctor   # validate tools and configuration\n"
                "  framekit --help   # see all commands"
            ),
        )
    )
...
@click.command(
    ...
    help=tr(
        "cli.init.help",
        default=(
            "Initialise a Framekit workspace non-interactively.\n\n"
            "Drops a ``framekit.yaml`` in the current directory and (unless "
            "``--minimal`` is passed) creates an empty ``Presets/`` tree. "
            "This is the headless counterpart of ``framekit setup`` — it is "
            "safe to run from CI, Docker entrypoints, or shell scripts.\n\n"
            "Examples:\n"
            "  framekit init                            # create framekit.yaml + Presets/\n"
            "  framekit init --force                    # overwrite existing files\n"
            "  framekit init --minimal                  # no Presets, blank vault config\n"
            "  framekit init --token eyJ.abc.xyz        # pre-fill TMDb v4 read token\n"
            "\n"
            "For an interactive walk-through use ``framekit setup`` instead."
        ),
    ),
)
@click.option(
    ...
    help=tr(
        "cli.init.force",
        default="Overwrite framekit.yaml if it already exists.",
    ),
)
@click.option(
    "--minimal",
...
def init_command(
    ...
    """Drop a project-local framekit.yaml (and presets) in ``target_dir``."""
