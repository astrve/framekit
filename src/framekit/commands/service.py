"""framekit service — service lifecycle management (Windows + Linux).

Subcommands:
    install    Install Framekit as a Windows service or scheduled task.
...
@click.group(
    "service",
    help=(
        "Manage the Framekit background service.\n\n"
        "Install Framekit as a persistent background service:\n\n"
        "    framekit service install\n\n"
        "Windows modes:\n"
        ...
        "Linux mode:\n"
        "  systemd — current-user unit (~/.config/systemd/user/framekit.service)."
    ),
)
def service_group() -> None:
    """Framekit service management commands."""
...
def install_command(mode: str, host: str, port: int) -> None:
    ...
    if platform_kind == "windows":
        ...
        from framekit.core.service.linux import install_systemd_user
    ...
    click.echo(msg)
    click.echo(
        "\nTo verify: framekit service status\n"
        "To start:  framekit service start\n"
        "Logs:      framekit service logs"
    )
...
@click.confirmation_option(
    prompt="Remove the Framekit service/task? This will stop it if running."
)
def uninstall_command(mode: str) -> None:
    """Remove installed Framekit service unit/task."""
    platform_kind = _platform_kind()
    ...
    if platform_kind == "windows":
        ...
        from framekit.core.service.linux import uninstall_systemd_user
...
def start_command(mode: str) -> None:
    ...
    if platform_kind == "windows":
        if mode == "systemd":
            ...
            sys.exit(1)
        from framekit.core.paths import get_service_dir
        from framekit.core.service.windows import start_service_sc, start_task
        ...
        from framekit.core.service.linux import start_systemd_user
...
def stop_command(mode: str) -> None:
    ...
    if platform_kind == "windows":
        if mode == "systemd":
            ...
            sys.exit(1)
        from framekit.core.paths import get_service_dir
        from framekit.core.service.windows import stop_service_sc, stop_task
        ...
        from framekit.core.service.linux import stop_systemd_user
...
def restart_command(mode: str) -> None:
    """Stop then start the Framekit service."""
    platform_kind = _platform_kind()
    ...
    if platform_kind == "windows":
        ...
        from framekit.core.service.linux import restart_systemd_user
...
def status_command(host: str, port: int, output_json: bool) -> None:
    """Show the current Framekit service status."""
    ...
    if platform_kind == "windows":
        ...
        from framekit.core.service.linux import (
            query_http_status,
    ...
    if http_status and not auth_required:
        ...
        click.echo(
            "Hint: authenticate with 'framekit user' or query the API with a JWT token."
        )
        ...
        if state:
            ...
            click.echo("Framekit service is not running (HTTP unreachable).")
        ...
        if not sc_state and not task_status_val:
            click.echo(
                "Framekit service is not running "
...
def logs_command(follow: bool, lines: int, stderr: bool) -> None:
    """Tail or follow the Framekit service log."""
    platform_kind = _platform_kind()

    if platform_kind == "linux":
        from framekit.core.service.linux import journal_logs
    ...
    from framekit.core.paths import get_service_dir
    from framekit.core.service.windows import follow_file, tail_file
    ...
    if follow:
        ...
        if not recent:
            click.echo(
                ...
                "the first task run. Start it with: schtasks /Run /TN Framekit"
            )
        else:
            for line in recent:
                click.echo(line)
...
def _resolve_windows_run_mode(mode: str) -> str:
    ...
    from framekit.core.service.windows import query_sc_status
    sc = query_sc_status()
    if "error" not in sc:
        return "sc"
    return "task"
