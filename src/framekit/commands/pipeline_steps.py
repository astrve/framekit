...
def _resolve_prez_templates_for_pipeline(
    *,
    ...
    from framekit.commands.prez import (
...
def _encoder_step(work_folder: Path, settings: dict) -> int:
    ...
    Returns:
        Exit code (0 for success, non-zero for failure)
    """
    from framekit.modules.encoder import EncoderService, PresetLoader
...
def _resolve_pipeline_encoder_preset(preset_name: str, loader):
    from framekit.modules.encoder import PresetLoader
...
def _resolve_pipeline_upload_description(
    *,
    ...
    description = "Release created with Framekit"
...
def _pipeline_best_effort_dupe_check(tracker_name: str, metadata: TorrentMetadata) -> None:
    ...
    try:
        ...
        from framekit.modules.upload.service import UploadService as _US

        first_config = _US()._load_tracker_config(tracker_name)  # pyright: ignore[reportPrivateUsage]
        from framekit.modules.upload.adapters.gazelle import GazelleAdapter
        from framekit.modules.upload.adapters.unit3d import UNIT3DAdapter
...
def _upload_step(
    ...
    """
    from framekit.modules.upload.image_host import ImageHostService, inject_images_into_bbcode
    from framekit.modules.upload.metadata_extractor import NFOData, NFOParser, ReleaseParser
