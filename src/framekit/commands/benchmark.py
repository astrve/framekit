...
from framekit.core.performance import get_metrics
...
def _benchmark_import_time(iterations: int) -> dict[str, Any]:
    ...
    for i in range(iterations):
        ...
        modules_to_clear = [m for m in sys.modules if m.startswith("framekit")]
        for mod in modules_to_clear:
            if mod not in ["framekit", "framekit.commands", "framekit.commands.benchmark"]:
...
@click.command(
    ...
    help=tr(
        "cli.benchmark.help",
        default=(
            "Run performance benchmarks and analyze Framekit's speed.\n\n"
            ...
            "Quick examples:\n"
            "  fk doctor benchmark                     # Run basic benchmarks\n"
            "  fk doc benchmark --folder <path>        # Include file scanning tests\n"
            "  fk doctor benchmark --iterations 5      # More iterations for accuracy\n"
            "  fk doc benchmark --export results.json  # Save results to file\n\n"
            ...
            "Note: This is a subcommand of 'doctor'. Use 'fk doctor' for health checks."
        ),
    ),
)
@click.option(
...
def benchmark_command(
    ...
    console.print()
    console.print("[bold cyan]Framekit Performance Benchmark[/bold cyan]")
    console.print()
