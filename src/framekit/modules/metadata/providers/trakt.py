...
from framekit.modules.metadata.providers.trakt_models import TraktIds, TraktMovie, TraktShow
...
class TraktProvider(MetadataProvider):
    ...
    def _make_request(
        ...
        try:
            ...
            raise FramekitMetadataError(f"Trakt authentication failed: {e}") from e
        except HttpRateLimitError:
            raise
        except HttpStatusError as e:
            if e.status_code == 404:
                return None
            raise FramekitMetadataError(f"Trakt API error: {e}") from e
        except HttpError as e:
            raise FramekitMetadataError(f"Trakt request error: {e}") from e
    ...
    def fetch_movie(self, candidate: MetadataCandidate) -> MovieMetadata:
        ...
        try:
            ...
            if not data:
                raise FramekitMetadataError(f"Trakt movie not found: {candidate.provider_id}")
            ...
            raise FramekitMetadataError(f"Failed to fetch Trakt movie: {e}") from e
    ...
    def fetch_episode(self, candidate: MetadataCandidate) -> EpisodeMetadata:
        ...
        try:
            ...
            if not show_data:
                raise FramekitMetadataError(f"Trakt show not found: {candidate.provider_id}")
            ...
            raise FramekitMetadataError(f"Failed to fetch Trakt episode: {e}") from e
    ...
    def fetch_season(self, candidate: MetadataCandidate) -> SeasonMetadata:
        ...
        try:
            ...
            raise FramekitMetadataError(f"Failed to fetch Trakt season: {e}") from e
