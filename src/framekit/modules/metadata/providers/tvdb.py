...
It handles authentication, rate limiting, and data transformation to Framekit's
...
from framekit.core.http import HttpAuthError, HttpClient, HttpError, HttpRateLimitError
...
class TVDBProvider(MetadataProvider):
    ...
    def _authenticate(self) -> None:
        ...
        Raises:
            FramekitMetadataError: If authentication fails
        """
        ...
        try:
            ...
            if not self.token:
                raise FramekitMetadataError("TVDB authentication failed: no token in response")

            logger.debug("TVDB authentication successful")

        except HttpAuthError as e:
            raise FramekitMetadataError(f"TVDB authentication failed: {e}") from e
        except HttpError as e:
            raise FramekitMetadataError(f"TVDB authentication error: {e}") from e
    ...
    def _make_request(
        ...
        try:
            ...
            raise FramekitMetadataError(f"TVDB API request failed: {e}") from e
    ...
    def fetch_movie(self, candidate: MetadataCandidate) -> MovieMetadata:
        ...
        Raises:
            FramekitMetadataError: TVDB does not support movies
        """
        raise FramekitMetadataError("TVDB does not support movie metadata")
    ...
    def fetch_episode(self, candidate: MetadataCandidate) -> EpisodeMetadata:
        ...
        Raises:
            FramekitMetadataError: On fetch failure
        """
        series_id, season_num, episode_num = self._resolve_episode_lookup(candidate)
        if season_num is None or episode_num is None:
            raise FramekitMetadataError("Season and episode numbers required for episode fetch")
        ...
        try:
            ...
            raise FramekitMetadataError(f"Failed to fetch TVDB episode: {e}") from e
    ...
    def fetch_season(self, candidate: MetadataCandidate) -> SeasonMetadata:
        ...
        if not season_num:
            raise FramekitMetadataError("Season number required for season fetch")
        ...
        try:
            ...
            raise FramekitMetadataError(f"Failed to fetch TVDB season: {e}") from e
