"""Webhook dispatch for Framekit events.
...
def _build_discord_payload(
    ...
    *,
    ...
    if title_template:
        ...
        embed_title = f"Framekit — {event.replace('.', ' ').title()}"
    ...
    if body_template:
        ...
        if event.startswith("job."):
            ...
            args = context["args"]
            cmd = f"`framekit {module}{' ' + args if args else ''}`"
    ...
    try:
        ...
        with httpx.Client(timeout=10.0) as client:
            response = client.post(
                webhook.url,
                json=payload,
                headers={"Content-Type": "application/json", "User-Agent": "Framekit/2.0"},
            )
