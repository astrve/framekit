...
Framekit exposes a public extension point named ``framekit.modules``. Any
installed Python distribution that declares an entry-point under this group
can contribute a new CLI subcommand without forking Framekit:

```toml
# pyproject.toml of a third-party package, e.g. ``framekit-plugin-anilist``
[project.entry-points."framekit.modules"]
anilist = "framekit_plugin_anilist:register"
```
...
ENTRY_POINT_GROUP = "framekit.modules"
...
def iter_plugin_entry_points() -> list[importlib_metadata.EntryPoint]:
    """Return every entry-point declared under ``framekit.modules``.
...
def load_plugins(cli: click.Group) -> list[str]:
    """Discover and register every ``framekit.modules`` plugin on ``cli``.
    ...
    Args:
        cli: The Click root group exposed by ``framekit.commands.main``.
    ...
    for entry_point in iter_plugin_entry_points():
        ...
        if not is_allowed:
            logger.info(
                f"Skipping framekit plugin {name!r} from distribution "
        ...
        try:
            ...
            logger.warning(
                f"Skipping framekit plugin {name!r}: failed to import "
        ...
        if not callable(register):
            logger.warning(
                f"Skipping framekit plugin {name!r}: entry-point target "
        ...
        try:
            ...
            logger.warning(
                f"Skipping framekit plugin {name!r}: register() raised "
                f"({exc.__class__.__name__}: {exc})"
            )
            continue

        loaded.append(name)
        logger.debug(f"Loaded framekit plugin: {name}")
...
def list_installed_plugins() -> list[dict[str, Any]]:
    ...
    Useful for ``framekit doctor`` or a future ``framekit plugins`` listing.
