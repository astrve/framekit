...
* Cached for 24 h in ``~/.cache/framekit/update_check.json`` (per-user).
* Disabled via ``FRAMEKIT_DISABLE_UPDATE_CHECK=1`` or by the
...
PYPI_URL = "https://pypi.org/pypi/framekit-cli/json"
CACHE_TTL_SECONDS = 24 * 60 * 60
NETWORK_TIMEOUT_SECONDS = 2.0
DISABLE_ENV_VAR = "FRAMEKIT_DISABLE_UPDATE_CHECK"
...
def _installed_version() -> str | None:
    """Return the installed ``framekit-cli`` version, or ``None`` when unknown."""
    for dist_name in ("framekit-cli", "framekit"):
        try:
...
def show_update_banner_if_available() -> None:
    ...
    try:
        ...
        console.print(
            f"[yellow]▲ Framekit {info.latest} is available "
            f"(installed {info.installed}). Upgrade with:[/yellow] "
            f"[cyan]pip install -U framekit-cli[/cyan]"
        )
    except Exception:
        # Fallback to a plain stderr line if rich is somehow unavailable.
        sys.stderr.write(
            f"Framekit {info.latest} available (installed {info.installed}). "
            f"Run: pip install -U framekit-cli\n"
        )
