"""Safe subprocess wrapper used by every external-tool invocation in Framekit.
...
def resolve_tool(name: str) -> str:
    ...
    if resolved is None:
        raise MissingToolError(
            ...
            "path in framekit.yaml under `tools:`, then re-run "
            "`fk doctor --check-tools` to verify.",
            tool=name,
            argv=(name,),
        )
    return resolved
