...
## [1.1.2] - NFO & CleanMKV Polish

### Added

- **Renamer term picker** – New interactive "terms to keep" selector available via `fk renamer --select-terms` (and from `fk pipeline --select-terms`). The picker scans the target folder, groups every detected token by category (episode codes, year, language, resolution, source, video codec, audio codec, HDR, team, free‑form / other) and pre‑checks every entry. Technical tokens that the renamer always rewrites from MediaInfo (resolution, codec, language tag, HDR marker, episode code, year) are shown as **locked** and cannot be unticked — so accidentally stripping `S01E03` or `1080P` is impossible. The user only really decides on free‑form tokens like `DSNP`, `INTERNAL`, `PROPER`, custom team suffixes, etc. Unticked entries are translated into the existing `--remove-term` plumbing, so behaviour downstream is unchanged. The picker opens automatically when running interactively without `--remove-term`/`--apply`/`--dry-run`; pass `--no-select-terms` to bypass it. A new module `framekit.modules.renamer.term_selector` exposes the inventory programmatically (`collect_terms`, `derive_remove_terms`, `TermInventory`, `TermEntry`), and its tokenisation pipeline is intentionally identical to the renamer's own (same `RAW_REPLACEMENTS`, same `COMPOUND_TOKENS`) so the user never sees fragments the renamer cannot act on.
- **NFO output mode (`global` / `per_file` / `both`)** – `fk nfo` now supports a new `--mode` flag (and `modules.nfo.mode` setting, default `global`). In `global` mode (existing behaviour) one NFO is written at the release root. In `per_file` mode, one NFO is written next to each `.mkv` using the MKV stem as the base name — useful for season packs uploaded as separate releases, or when a tracker expects per‑episode NFOs. `both` runs the two passes in a single invocation, sharing the same metadata fetch so TMDb is not queried twice. When the global metadata context carries a `metadata_episode_map`, each per‑file render is automatically scoped to that episode's metadata. Available from `fk pipeline --nfo-mode <value>` as well. When running interactively without `--mode`/`--write`, an additional picker step asks which mode to use. New service methods `NfoService.build_per_file` / `NfoService.write_per_file` back the feature; `commands/nfo.py` gained `_normalize_nfo_mode` and `_choose_nfo_mode` helpers.
- **`Encode Settings` in detailed NFO templates** – Every `*_detailed.{en,fr,es}.jinja2` template (6 files: movie, single‑episode, series × 3 locales) now exposes the raw `video_encoding_settings` string from MediaInfo on its own line, right after the `HDR` line in the `FILE` / `FICHIER` / `ARCHIVO` (or per‑episode) section. Labels: `Encode Settings` (EN), `Paramètres d'encodage` (FR), `Ajustes de codificación` (ES). The data was already populated by the scanner — only the templates needed the new line. The line is automatically suppressed by the existing `ui.line` macro when MediaInfo did not surface any encoding settings, so unaffected sources (e.g. remuxes without an encode log) render as before.

- **Single‑file metadata and NFO support** – You can now pass a path to a single `.mkv` file directly to the `fk md` (metadata) and `fk nfo` commands. When a file is provided, Framekit scans its parent directory and builds a release containing only that episode. This makes it possible to resolve metadata or generate an NFO for one episode without placing it into its own folder.
- **New translation keys** – Added `cleanmkv.error.invalid_file_type` to report when a non‑MKV file is supplied, and `cleanmkv.warning.different_bitrates` to warn when multiple audio tracks differ only by bitrate. Translations have been added for English, French and Spanish.
- **CleanMKV interactive selector pre‑checks every track** – When using the interactive track selector (`fk cleanmkv <folder>` without a preset), every detected audio and subtitle track is now pre‑selected. Users only need to un‑tick the tracks they want to drop, which is materially faster than ticking each one in for releases that mostly keep the source layout.
- **Explicit "no default" honoured by CleanMKV planner** – `CleanPreset` gained two opt‑in flags (`audio_default_explicit`, `subtitle_default_explicit`) that the wizard and track selector now set to `True`. When set, picking "None" as the default audio or subtitle track is treated as an explicit user decision and the planner no longer falls back to the source file's `is_default` flag. Builtin presets keep their previous fallback behaviour for backward compatibility.

### Changed

- **Improved audio track de‑duplication** – CleanMKV now de‑duplicates audio tracks based solely on language, codec and channel layout. Both the bitrate and the track title are ignored when computing the reference key. This prevents duplicate entries when the same mix is encoded at different bitrates or when titles differ subtly. If otherwise identical tracks are found with different bitrates, a warning is shown in the CLI and only one entry is offered.
- **Version bump** – Updated the project version to `1.1.2` in package metadata and runtime version reporting. The HTTP `User-Agent` (`framekit.core.http.DEFAULT_USER_AGENT`) and the `_get_version()` fallback in `framekit.commands.main` are now also synchronised with the package version (previously `1.0.11.1` and `1.1.0` respectively).
- **Documentation** – The README now includes guidance on obtaining a TMDb read access token or API key and storing it via `fk md -t` or `fk setup`. This should help users avoid “Invalid API key” errors when resolving metadata.

### Fixed

- **Single‑file handling in metadata and NFO** – Passing a path to an individual MKV file to `fk md` or `fk nfo` previously resulted in a “Folder not found” error. These commands now detect single files, validate that they are MKV containers and operate on just that file.
- **`fk doctor` no longer launches the MediaInfo GUI on macOS** – On systems where MediaInfo is installed as an `.app` bundle (macOS DMG install, occasionally Windows), running `fk doctor` would invoke the GUI executable inside the bundle instead of returning a version string, causing the MediaInfo window to pop up. The tool resolver now prefers CLI‑specific binary names (`mediainfo`, `mediainfo-cli`, `MediaInfoCLI`), skips `.app` bundles when probing versions, surfaces a clear "GUI‑only binary detected" warning when no CLI alternative is available, and uses `CREATE_NO_WINDOW` on Windows so probing tools never flashes a console window.
- **CleanMKV no longer picks a random default subtitle when "None" was chosen** – When a user explicitly selected no default subtitle through the interactive track selector or wizard, the planner used to silently fall back to the first track marked `is_default` in the source file, occasionally producing remuxed files with an unintended default subtitle. The planner now respects an explicit "no default" choice end‑to‑end.
