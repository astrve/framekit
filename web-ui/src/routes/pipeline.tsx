...
const PIPELINE_PRESETS = [
  { id: "movie-core", label: "Movie Core", args: "run --profile movie --input \"\" --workspace \"\" --dry-run" },
  { id: "series-core", label: "Series Core", args: "run --profile series --input \"\" --workspace \"\" --dry-run" },
  { id: "watch-encode", label: "Watch + Encode", args: "run --steps watch,inspect,encode --input \"\" --workspace \"\" --dry-run" },
] as const;

function validatePipelineArgs(argsText: string): string | null {
  const trimmed = argsText.trim();
  if (!trimmed) {
    return "Les arguments pipeline sont requis.";
  }
  if (!trimmed.includes("--input")) {
    return "Ajoute --input pour indiquer la source.";
  }
  if (!trimmed.includes("--workspace")) {
    return "Ajoute --workspace pour isoler l'exécution.";
  }
  return null;
}
...
export function PipelinePage() {
  ...
  const [argsText, setArgsText] = useState<string>(PIPELINE_PRESETS[0].args);
  ...
  const previewCommand = useMemo(() => `framekit pipeline ${argsText.trim()}`.trim(), [argsText]);
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-border bg-card p-5">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Workflow pipeline local avec presets, validation et observabilité job.</p>
      </section>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Prépare et exécute `framekit pipeline ...`</CardDescription>
        </CardHeader>
        <CardContent className="space-y-3">
          ...
          <label className="space-y-1 text-sm" htmlFor="pipeline-args">
            Arguments
            <Input id="pipeline-args" value={argsText} onChange={(e) => setArgsText(e.target.value)} />
          </label>
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              type="button"
              onClick={() => {
                const validationError = validatePipelineArgs(argsText);
                if (validationError) {
            ...
            <Button type="button" variant="ghost" onClick={() => setArgsText(PIPELINE_PRESETS[0].args)}>
              Reset preset
            </Button>
      ...
      {runResult ? (
        <Card>
          <CardHeader>
            <CardTitle>Résultat pipeline</CardTitle>
