"""Tests for framekit.core.service.linux — Linux systemd user-service helpers."""
...
def _import_linux():
    ...
    from framekit.core.service import linux
...
class TestBuildUnit:
    def test_build_exec_start_uses_framekit_binary_when_found(self):
        svc = _import_linux()
        with patch("shutil.which", return_value="/usr/local/bin/framekit"):
            cmd = svc._build_exec_start("127.0.0.1", 8000)
        assert cmd.startswith("/usr/local/bin/framekit serve")
    ...
    def test_build_exec_start_falls_back_to_python_module(self):
        ...
        with patch("shutil.which", return_value=None), patch.object(
            ...
            cmd = svc._build_exec_start("127.0.0.1", 8000)
        assert "/opt/python/bin/python3 -m framekit serve" in cmd

    def test_build_unit_text_contains_systemd_sections(self):
        svc = _import_linux()
        with patch("shutil.which", return_value="/usr/bin/framekit"):
            text = svc._build_unit_text("127.0.0.1", 8000)
        assert "[Unit]" in text
        assert "[Service]" in text
        assert "[Install]" in text
        assert "ExecStart=/usr/bin/framekit serve --host 127.0.0.1 --port 8000" in text
...
class TestInstallSystemdUser:
    ...
    def test_success_writes_unit_and_runs_reload_enable(self, monkeypatch, tmp_path):
        ...
        assert (tmp_path / "framekit.service").exists()
...
class TestUninstallSystemdUser:
    def test_success_even_when_unit_missing(self, monkeypatch, tmp_path):
        ...
        with patch.object(svc, "_find_systemctl", return_value="/bin/systemctl"), patch.object(
            ...
            "_run",
            side_effect=[
                (False, "Unit framekit.service could not be found."),
                (True, "ok"),
            ],
        ):
