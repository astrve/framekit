"""Tests for the ``framekit.modules`` plugin discovery layer.
...
from framekit.core import plugins as plugin_module
...
def test_list_installed_plugins_returns_metadata_rows() -> None:
    ...
    fake_dist.name = "framekit-plugin-demo"
    fake_dist.version = "1.2.3"
    ep = MagicMock()
    ep.name = "demo"
    ep.value = "framekit_plugin_demo:register"
    ep.dist = fake_dist
...
def test_entry_point_group_constant() -> None:
    ...
    assert plugin_module.ENTRY_POINT_GROUP == "framekit.modules"
