...
def test_settings_store_uses_user_config_when_no_project_file(tmp_path, monkeypatch) -> None:
    ...
    assert store.path == config_dir / "swirrl.yaml"
    assert store.path.exists()
    assert not (workspace / "swirrl.yaml").exists()
...
def test_existing_project_settings_file_wins_over_user_config(tmp_path, monkeypatch) -> None:
    ...
    project_config = project / "swirrl.yaml"
...
def test_configured_global_settings_path_is_used_when_no_project_file(
    ...
    from swirrl.core.paths import get_settings_path, set_configured_global_settings_path

    workspace = tmp_path / "release"
    config_dir = tmp_path / "user-config"
    cache_dir = tmp_path / "user-cache"
    custom_settings = tmp_path / "custom-config" / "swirrl.yaml"
    workspace.mkdir()
...
def test_swirrl_config_env_overrides_project_and_user_config(tmp_path, monkeypatch) -> None:
    from swirrl.core.paths import get_settings_path
    ...
    workspace.mkdir()
    (workspace / "swirrl.yaml").write_text("general:\n  locale: fr\n", encoding="utf-8")

    monkeypatch.chdir(workspace)
    monkeypatch.setenv("SWIRRL_CONFIG", str(override))
    _patch_platform_dirs(monkeypatch, config_dir=config_dir, cache_dir=cache_dir)
...
def test_settings_store_merges_module_yaml_next_to_active_config(tmp_path, monkeypatch) -> None:
    ...
    settings_file = config_dir / "swirrl.yaml"
