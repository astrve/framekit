...
def test_upload_assistant_saves_tracker_yaml_without_token(tmp_path, monkeypatch) -> None:
    ...
    assert "OURO_TRACKER_MY_TRACKER_TOKEN" in content
    ...
    (trackers_dir / "my-tracker.yaml").write_text(
        "\n".join(
            [
                ...
                "auth:",
                "  type: bearer",
                "  token_env: OURO_TRACKER_MY_TRACKER_TOKEN",
            ]
        ),
        encoding="utf-8",
    )
    monkeypatch.setenv("OURO_CONFIG", str(settings_file))
    monkeypatch.setenv("OURO_TRACKER_MY_TRACKER_TOKEN", "runtime-token")
