"""Linux command-dispatch tests for ``swirrl service`` CLI."""
...
def test_install_auto_uses_systemd_on_linux(monkeypatch) -> None:
    ...
    runner = CliRunner()
    with patch(
        "swirrl.core.service.linux.install_systemd_user",
...
def test_start_auto_uses_systemd_on_linux(monkeypatch) -> None:
    ...
    runner = CliRunner()
    with patch(
        "swirrl.core.service.linux.start_systemd_user",
...
def test_uninstall_systemd_on_linux(monkeypatch) -> None:
    ...
    runner = CliRunner()
    with patch(
        "swirrl.core.service.linux.uninstall_systemd_user",
...
def test_logs_linux_uses_journal(monkeypatch) -> None:
    ...
    runner = CliRunner()
    with patch(
        "swirrl.core.service.linux.journal_logs",
...
def test_status_linux_json_includes_systemd_block(monkeypatch, tmp_path) -> None:
    ...
    runner = CliRunner()
    with patch(
        "swirrl.core.service.linux.query_http_status",
        return_value=None,
    ), patch(
        "swirrl.core.service.linux.read_service_state",
        return_value=None,
    ), patch(
        "swirrl.core.service.linux.query_systemd_status",
        return_value={"state": "ACTIVE", "mainpid": "123"},
    ), patch(
        "swirrl.core.paths.get_service_dir",
