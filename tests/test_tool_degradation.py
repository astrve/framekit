...
This test suite verifies that Swirrl handles missing external tools
...
class TestToolRegistry:
    ...
    def test_missing_mkvmerge_returns_unavailable_status(self, tmp_path):
        ...
        with patch("swirrl.core.tools.shutil.which", return_value=None):
    ...
    def test_tool_exists_but_not_executable(self, tmp_path):
        ...
        with patch("swirrl.core.tools.shutil.which", return_value=str(fake_tool)):
            with patch("swirrl.core.tools._run_version_command") as mock_run:
...
class TestCleanMkvWithMissingTools:
    ...
    def test_scanner_raises_helpful_error_when_mkvmerge_missing(self, tmp_path):
        ...
        from swirrl.modules.cleanmkv.scanner import scan_mkv_file

        test_file = tmp_path / "test.mkv"
        test_file.write_bytes(b"fake mkv content")

        with patch("swirrl.core.tools.ToolRegistry.resolve_tool_path", return_value=None):
...
class TestEncoderWithMissingTools:
    ...
    def test_encoder_service_init_fails_gracefully_without_ffmpeg(self):
        ...
        from swirrl.modules.encoder.models import (
            AdvancedConfig,
            AudioConfig,
            ChapterConfig,
            EncodePreset,
        ...
        from swirrl.modules.encoder.service import EncoderService
        ...
        with patch("swirrl.modules.encoder.service.subprocess.run") as mock_run:
    ...
    def test_validator_detects_missing_ffmpeg(self):
        ...
        from swirrl.modules.encoder.validator import EncoderValidator

        with patch("swirrl.modules.encoder.validator.subprocess.run") as mock_run:
...
class TestPartialToolAvailability:
    ...
    def test_mkvmerge_available_ffmpeg_missing(self):
        ...
        with patch("swirrl.core.tools.shutil.which", side_effect=mock_which):
            with patch("swirrl.core.tools._run_version_command") as mock_run:
...
class TestToolErrorMessages:
    ...
    def test_macos_app_bundle_detection(self):
        ...
        with patch("swirrl.core.tools.shutil.which", return_value=fake_app_path):
            registry = ToolRegistry()
            status = registry.get_status("mkvmerge")
...
class TestToolRegistryEdgeCases:
    ...
    def test_tool_version_command_timeout(self):
        ...
        with patch("swirrl.core.tools.shutil.which", return_value="/usr/bin/mkvmerge"):
            with patch("swirrl.core.tools.subprocess.run") as mock_run:
                import subprocess
