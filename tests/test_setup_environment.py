...
class TestFolderCreation:
    ...
    @patch("swirrl.commands.setup.Path.mkdir")
    @patch("swirrl.commands.setup.print_warning")
...
class TestFirstTimeSetupDetection:
    ...
    @patch("swirrl.commands.setup.sys.stdin")
    @patch("swirrl.commands.setup.sys.stdout")
    ...
    @patch("swirrl.commands.setup.sys.stdin")
    @patch("swirrl.commands.setup.sys.stdout")
    @patch("swirrl.commands.setup.sys.argv", ["swirrl", "setup"])
    @patch("swirrl.commands.setup.SettingsStore")
    ...
    @patch("swirrl.commands.setup.sys.stdin")
    @patch("swirrl.commands.setup.sys.stdout")
    @patch("swirrl.commands.setup.sys.argv", ["swirrl", "--help"])
    @patch("swirrl.commands.setup.SettingsStore")
...
class TestEnvironmentCompatibility:
    ...
    def test_config_dir_windows_style(self, mock_get_config: Mock, tmp_path: Path):
        ...
        config_dir = tmp_path / "AppData" / "Roaming" / "Swirrl"
        mock_get_config.return_value = config_dir
        project_root = tmp_path / "project"
    ...
    def test_config_dir_unix_style(self, mock_get_config: Mock, tmp_path: Path):
        ...
        config_dir = tmp_path / ".config" / "swirrl"
        mock_get_config.return_value = config_dir
        project_root = tmp_path / "project"
