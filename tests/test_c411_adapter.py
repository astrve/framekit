"""Tests for c411 adapter upload error handling."""

from __future__ import annotations

from framekit.core.http import HttpResponse
from framekit.modules.upload.adapters.c411 import C411Adapter
...
def test_c411_upload_parses_403_json_errors(monkeypatch, tmp_path):
    ...
    adapter = C411Adapter(_make_config())
    ...
    def _fake_request(*_args, **_kwargs):
        return HttpResponse(
            url="https://c411.org/api/torrents",
            status_code=403,
            headers={},
...
def test_c411_upload_parses_422_json_errors(monkeypatch, tmp_path):
    torrent_path = tmp_path / "sample.torrent"
    torrent_path.write_bytes(
        b"d8:announce13:https://x/4:infod6:lengthi1e4:name1:a12:piece lengthi16e6:pieces20:aaaaaaaaaaaaaaaaaaaaee"
    )
...
def test_c411_validate_credentials_detects_missing_upload_scope(monkeypatch):
    adapter = C411Adapter(_make_config())

    def _fake_get(*_args, **_kwargs):
        return HttpResponse(
            url="https://c411.org/api/categories",
            status_code=200,
            headers={},
            body=b'{"data":[]}',
        )
...
def test_c411_validate_credentials_accepts_payload_validation_errors(monkeypatch):
    adapter = C411Adapter(_make_config())
